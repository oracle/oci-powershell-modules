...
{
    [Cmdlet("Move", "OCIDatabaseSchedulingPolicyCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeSchedulingPolicyCompartmentResponse) })]
    public class MoveOCIDatabaseSchedulingPolicyCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move scheduling policy to a different compartment", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move scheduling policy to a different compartment", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeSchedulingPolicyCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeSchedulingPolicyCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeSchedulingPolicyCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeSchedulingPolicyCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeSchedulingPolicyCompartmentResponse response;
