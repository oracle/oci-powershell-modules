...
{
    [Cmdlet("New", "OCIDatabaseSchedulingPolicy", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.SchedulingPolicy), typeof(Oci.DatabaseService.Responses.CreateSchedulingPolicyResponse) })]
    public class NewOCIDatabaseSchedulingPolicy : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create Scheduling Policy.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create Scheduling Policy.", ParameterSetName = Default)]
        ...
        private void HandleOutput(CreateSchedulingPolicyRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateSchedulingPolicy(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateSchedulingPolicy(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.SchedulingPolicy);
        }

        private CreateSchedulingPolicyResponse response;
