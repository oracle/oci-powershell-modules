...
{
    [Cmdlet("Get", "OCIOsmanagementhubLifecycleEnvironmentsList")]
    [OutputType(new System.Type[] { typeof(Oci.OsmanagementhubService.Models.LifecycleEnvironmentCollection), typeof(Oci.OsmanagementhubService.Responses.ListLifecycleEnvironmentsResponse) })]
    public class GetOCIOsmanagementhubLifecycleEnvironmentsList : OCILifecycleEnvironmentCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the lifecycle environment.")]
...
Example: `3`")]
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to return only the lifecycle environments that match the display name given.")]
        public System.Nullable<Oci.OsmanagementhubService.Models.LifecycleEnvironment.LifecycleStateEnum> LifecycleState { get; set; }
        ...
        public System.Nullable<Oci.OsmanagementhubService.Requests.ListLifecycleEnvironmentsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListLifecycleEnvironmentsRequest
                {
                ...
                IEnumerable<ListLifecycleEnvironmentsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.LifecycleEnvironmentCollection, true);
                }
        ...
        {
            IEnumerable<ListLifecycleEnvironmentsResponse> DefaultRequest(ListLifecycleEnvironmentsRequest request) => Enumerable.Repeat(client.ListLifecycleEnvironments(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListLifecycleEnvironmentsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListLifecycleEnvironmentsResponse response;
        private delegate IEnumerable<ListLifecycleEnvironmentsResponse> RequestDelegate(ListLifecycleEnvironmentsRequest request);
