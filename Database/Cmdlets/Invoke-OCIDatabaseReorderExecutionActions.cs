...
{
    [Cmdlet("Invoke", "OCIDatabaseReorderExecutionActions", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExecutionWindow), typeof(Oci.DatabaseService.Responses.ReorderExecutionActionsResponse) })]
    public class InvokeOCIDatabaseReorderExecutionActions : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The execution window [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The execution window [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        public string ExecutionWindowId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to reorder the execution actions under this execution window resource.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to reorder the execution actions under this execution window resource.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ReorderExecutionActionsRequest
                {
        ...
        private void HandleOutput(ReorderExecutionActionsRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForReorderExecutionActions(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ReorderExecutionActions(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExecutionWindow);
        }

        private ReorderExecutionActionsResponse response;
