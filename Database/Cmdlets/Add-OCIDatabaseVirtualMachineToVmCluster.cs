...
{
    [Cmdlet("Add", "OCIDatabaseVirtualMachineToVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.VmCluster), typeof(Oci.DatabaseService.Responses.AddVirtualMachineToVmClusterResponse) })]
    public class AddOCIDatabaseVirtualMachineToVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to add Virtual Machines to the VM cluster.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to add Virtual Machines to the VM cluster.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new AddVirtualMachineToVmClusterRequest
                {
        ...
        private void HandleOutput(AddVirtualMachineToVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForAddVirtualMachineToVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.AddVirtualMachineToVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.VmCluster);
        }

        private AddVirtualMachineToVmClusterResponse response;
