...
{
    [Cmdlet("Invoke", "OCIDatabaseMountDbnodeSnapshot", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbnodeSnapshot), typeof(Oci.DatabaseService.Responses.MountDbnodeSnapshotResponse) })]
    public class InvokeOCIDatabaseMountDbnodeSnapshot : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to mount the snapshot for the provided Exadata Database Node.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to mount the snapshot for the provided Exadata Database Node.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new MountDbnodeSnapshotRequest
                {
        ...
        private void HandleOutput(MountDbnodeSnapshotRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForMountDbnodeSnapshot(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.MountDbnodeSnapshot(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbnodeSnapshot);
        }

        private MountDbnodeSnapshotResponse response;
