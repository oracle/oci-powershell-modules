...
{
    [Cmdlet("Get", "OCIRedisClusterNodesList")]
    ...
    public class GetOCIRedisClusterNodesList : OCIRedisClusterCmdlet
    {
        ...
        public System.Nullable<Oci.RedisService.Requests.ListRedisClusterNodesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListRedisClusterNodesRequest
                {
                ...
                IEnumerable<ListRedisClusterNodesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.RedisNodeCollection, true);
                }
        ...
        {
            IEnumerable<ListRedisClusterNodesResponse> DefaultRequest(ListRedisClusterNodesRequest request) => Enumerable.Repeat(client.ListRedisClusterNodes(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListRedisClusterNodesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListRedisClusterNodesResponse response;
        private delegate IEnumerable<ListRedisClusterNodesResponse> RequestDelegate(ListRedisClusterNodesRequest request);
