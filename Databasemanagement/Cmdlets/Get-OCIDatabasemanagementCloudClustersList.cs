...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudClustersList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudClusterCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudClustersResponse) })]
    public class GetOCIDatabasemanagementCloudClustersList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudClustersRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudClustersRequest
                {
                ...
                IEnumerable<ListCloudClustersResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudClusterCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudClustersResponse> DefaultRequest(ListCloudClustersRequest request) => Enumerable.Repeat(client.ListCloudClusters(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudClustersResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudClustersResponse response;
        private delegate IEnumerable<ListCloudClustersResponse> RequestDelegate(ListCloudClustersRequest request);
