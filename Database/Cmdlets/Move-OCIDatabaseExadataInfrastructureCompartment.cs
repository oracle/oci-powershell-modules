...
{
    [Cmdlet("Move", "OCIDatabaseExadataInfrastructureCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeExadataInfrastructureCompartmentResponse) })]
    public class MoveOCIDatabaseExadataInfrastructureCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move Exadata infrastructure to a different compartment", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move Exadata infrastructure to a different compartment", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeExadataInfrastructureCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeExadataInfrastructureCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeExadataInfrastructureCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeExadataInfrastructureCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeExadataInfrastructureCompartmentResponse response;
