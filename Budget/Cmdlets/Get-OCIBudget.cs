...
{
    [Cmdlet("Get", "OCIBudget", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.BudgetService.Models.Budget), typeof(Oci.BudgetService.Responses.GetBudgetResponse) })]
    public class GetOCIBudget : OCIBudgetCmdlet
    {
        ...
        private void HandleOutput(GetBudgetRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForBudget(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetBudget(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Budget);
        }

        private GetBudgetResponse response;
