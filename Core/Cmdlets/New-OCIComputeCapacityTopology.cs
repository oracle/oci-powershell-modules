...
{
    [Cmdlet("New", "OCIComputeCapacityTopology", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.ComputeCapacityTopology), typeof(Oci.CoreService.Responses.CreateComputeCapacityTopologyResponse) })]
    public class NewOCIComputeCapacityTopology : OCIComputeCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for creating a new compute capacity topology.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for creating a new compute capacity topology.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateComputeCapacityTopologyRequest
                {
        ...
        private void HandleOutput(CreateComputeCapacityTopologyRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateComputeCapacityTopology(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateComputeCapacityTopology(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ComputeCapacityTopology);
        }

        private CreateComputeCapacityTopologyResponse response;
