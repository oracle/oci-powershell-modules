...
{
    [Cmdlet("Invoke", "OCIVirtualNetworkBulkUpdateIpv6s", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.BulkUpdateIpv6sResponse) })]
    public class InvokeOCIVirtualNetworkBulkUpdateIpv6s : OCIVirtualNetworkCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details of the IPv6s to update.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details of the IPv6s to update.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new BulkUpdateIpv6sRequest
                {
        ...
        private void HandleOutput(BulkUpdateIpv6sRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForBulkUpdateIpv6s(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.BulkUpdateIpv6s(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private BulkUpdateIpv6sResponse response;
