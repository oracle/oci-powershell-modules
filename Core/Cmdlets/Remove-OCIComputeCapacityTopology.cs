...
{
    [Cmdlet("Remove", "OCIComputeCapacityTopology", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.High, DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.DeleteComputeCapacityTopologyResponse) })]
    public class RemoveOCIComputeCapacityTopology : OCIComputeCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the compute capacity topology.", ParameterSetName = StatusParamSet)]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (!ConfirmDelete("OCIComputeCapacityTopology", "Remove"))
            {
               return;
            }
            ...
            try
            {
                request = new DeleteComputeCapacityTopologyRequest
                {
        ...
        private void HandleOutput(DeleteComputeCapacityTopologyRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForDeleteComputeCapacityTopology(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.DeleteComputeCapacityTopology(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private DeleteComputeCapacityTopologyResponse response;
