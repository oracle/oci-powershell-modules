...
{
    [Cmdlet("Move", "OCIDatabaseExascaleDbStorageVaultSubscription", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeExascaleDbStorageVaultSubscriptionResponse) })]
    public class MoveOCIDatabaseExascaleDbStorageVaultSubscription : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a Exadata Database Storage Vault with a different subscription.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a Exadata Database Storage Vault with a different subscription.", ParameterSetName = Default)]
        public ChangeExascaleDbStorageVaultSubscriptionDetails ChangeExascaleDbStorageVaultSubscriptionDetails { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Exadata Database Storage Vault [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Exadata Database Storage Vault [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeExascaleDbStorageVaultSubscriptionRequest
                {
        ...
        private void HandleOutput(ChangeExascaleDbStorageVaultSubscriptionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeExascaleDbStorageVaultSubscription(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeExascaleDbStorageVaultSubscription(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeExascaleDbStorageVaultSubscriptionResponse response;
