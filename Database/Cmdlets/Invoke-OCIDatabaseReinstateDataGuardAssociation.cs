...
{
    [Cmdlet("Invoke", "OCIDatabaseReinstateDataGuardAssociation", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DataGuardAssociation), typeof(Oci.DatabaseService.Responses.ReinstateDataGuardAssociationResponse) })]
    public class InvokeOCIDatabaseReinstateDataGuardAssociation : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Data Guard association's [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Data Guard association's [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        public string DataGuardAssociationId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"A request to reinstate a database in a standby role.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"A request to reinstate a database in a standby role.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ReinstateDataGuardAssociationRequest
                {
        ...
        private void HandleOutput(ReinstateDataGuardAssociationRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForReinstateDataGuardAssociation(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ReinstateDataGuardAssociation(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DataGuardAssociation);
        }

        private ReinstateDataGuardAssociationResponse response;
