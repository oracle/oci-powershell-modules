...
{
    [Cmdlet("Get", "OCIMarketplacepublisherMarketsList")]
    [OutputType(new System.Type[] { typeof(Oci.MarketplacepublisherService.Models.MarketCollection), typeof(Oci.MarketplacepublisherService.Responses.ListMarketsResponse) })]
    public class GetOCIMarketplacepublisherMarketsList : OCIMarketplacePublisherCmdlet
    {
        ...
        public System.Nullable<Oci.MarketplacepublisherService.Requests.ListMarketsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListMarketsRequest
                {
                ...
                IEnumerable<ListMarketsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.MarketCollection, true);
                }
        ...
        {
            IEnumerable<ListMarketsResponse> DefaultRequest(ListMarketsRequest request) => Enumerable.Repeat(client.ListMarkets(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListMarketsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListMarketsResponse response;
        private delegate IEnumerable<ListMarketsResponse> RequestDelegate(ListMarketsRequest request);
