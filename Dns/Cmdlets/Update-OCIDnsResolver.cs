...
{
    [Cmdlet("Update", "OCIDnsResolver", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DnsService.Models.Resolver), typeof(Oci.DnsService.Responses.UpdateResolverResponse) })]
    public class UpdateOCIDnsResolver : OCIDnsCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"New data for the resolver.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"New data for the resolver.", ParameterSetName = Default)]
        ...
        private void HandleOutput(UpdateResolverRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateResolver(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateResolver(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Resolver);
        }

        private UpdateResolverResponse response;
