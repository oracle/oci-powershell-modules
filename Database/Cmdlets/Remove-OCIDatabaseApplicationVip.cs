...
{
    [Cmdlet("Remove", "OCIDatabaseApplicationVip", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.High, DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.DeleteApplicationVipResponse) })]
    public class RemoveOCIDatabaseApplicationVip : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the application virtual IP (VIP) address.", ParameterSetName = StatusParamSet)]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (!ConfirmDelete("OCIDatabaseApplicationVip", "Remove"))
            {
               return;
            }
            ...
            try
            {
                request = new DeleteApplicationVipRequest
                {
        ...
        private void HandleOutput(DeleteApplicationVipRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForDeleteApplicationVip(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.DeleteApplicationVip(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private DeleteApplicationVipResponse response;
