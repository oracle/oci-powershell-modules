...
{
    [Cmdlet("Update", "OCIComputeGpuMemoryCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.ComputeGpuMemoryCluster), typeof(Oci.CoreService.Responses.UpdateComputeGpuMemoryClusterResponse) })]
    public class UpdateOCIComputeGpuMemoryCluster : OCIComputeCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The OCID of the compute GPU memory cluster.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The OCID of the compute GPU memory cluster.", ParameterSetName = Default)]
        public string ComputeGpuMemoryClusterId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Update compute GPU memory cluster details.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Update compute GPU memory cluster details.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpdateComputeGpuMemoryClusterRequest
                {
        ...
        private void HandleOutput(UpdateComputeGpuMemoryClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateComputeGpuMemoryCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateComputeGpuMemoryCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ComputeGpuMemoryCluster);
        }

        private UpdateComputeGpuMemoryClusterResponse response;
