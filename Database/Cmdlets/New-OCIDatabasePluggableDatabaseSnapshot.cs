...
{
    [Cmdlet("New", "OCIDatabasePluggableDatabaseSnapshot", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabaseSnapshot), typeof(Oci.DatabaseService.Responses.CreatePluggableDatabaseSnapshotResponse) })]
    public class NewOCIDatabasePluggableDatabaseSnapshot : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a pluggable database snapshot.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a pluggable database snapshot.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreatePluggableDatabaseSnapshotRequest
                {
        ...
        private void HandleOutput(CreatePluggableDatabaseSnapshotRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreatePluggableDatabaseSnapshot(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreatePluggableDatabaseSnapshot(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabaseSnapshot);
        }

        private CreatePluggableDatabaseSnapshotResponse response;
