...
{
    [Cmdlet("Update", "OCIDatabaseConsoleConnection", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ConsoleConnection), typeof(Oci.DatabaseService.Responses.UpdateConsoleConnectionResponse) })]
    public class UpdateOCIDatabaseConsoleConnection : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The OCID of the console connection.", ParameterSetName = StatusParamSet)]
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the specified database node console connection.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the specified database node console connection.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpdateConsoleConnectionRequest
                {
                    DbNodeId = DbNodeId,
        ...
        private void HandleOutput(UpdateConsoleConnectionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateConsoleConnection(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateConsoleConnection(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ConsoleConnection);
        }

        private UpdateConsoleConnectionResponse response;
