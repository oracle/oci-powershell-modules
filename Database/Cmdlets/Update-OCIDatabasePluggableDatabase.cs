...
{
    [Cmdlet("Update", "OCIDatabasePluggableDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabase), typeof(Oci.DatabaseService.Responses.UpdatePluggableDatabaseResponse) })]
    public class UpdateOCIDatabasePluggableDatabase : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to perform pluggable database update.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to perform pluggable database update.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpdatePluggableDatabaseRequest
                {
        ...
        private void HandleOutput(UpdatePluggableDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdatePluggableDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdatePluggableDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabase);
        }

        private UpdatePluggableDatabaseResponse response;
