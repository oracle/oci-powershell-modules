...
{
    [Cmdlet("Enable", "OCIDatabaseAutonomousDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.EnableAutonomousDatabaseManagementResponse) })]
    public class EnableOCIDatabaseAutonomousDatabaseManagement : OCIDatabaseCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableAutonomousDatabaseManagementRequest
                {
        ...
        private void HandleOutput(EnableAutonomousDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableAutonomousDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableAutonomousDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private EnableAutonomousDatabaseManagementResponse response;
