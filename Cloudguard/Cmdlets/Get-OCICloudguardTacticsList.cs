...
{
    [Cmdlet("Get", "OCICloudguardTacticsList")]
    [OutputType(new System.Type[] { typeof(Oci.CloudguardService.Models.TacticCollection), typeof(Oci.CloudguardService.Responses.ListTacticsResponse) })]
    public class GetOCICloudguardTacticsList : OCICloudGuardCmdlet
    {
        ...
        public System.Nullable<Oci.CloudguardService.Requests.ListTacticsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListTacticsRequest
                {
                ...
                IEnumerable<ListTacticsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.TacticCollection, true);
                }
        ...
        {
            IEnumerable<ListTacticsResponse> DefaultRequest(ListTacticsRequest request) => Enumerable.Repeat(client.ListTactics(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListTacticsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListTacticsResponse response;
        private delegate IEnumerable<ListTacticsResponse> RequestDelegate(ListTacticsRequest request);
