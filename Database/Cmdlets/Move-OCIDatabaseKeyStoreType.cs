...
{
    [Cmdlet("Move", "OCIDatabaseKeyStoreType", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeKeyStoreTypeResponse) })]
    public class MoveOCIDatabaseKeyStoreType : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to change the source of the encryption key for the database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to change the source of the encryption key for the database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeKeyStoreTypeRequest
                {
        ...
        private void HandleOutput(ChangeKeyStoreTypeRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeKeyStoreType(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeKeyStoreType(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeKeyStoreTypeResponse response;
