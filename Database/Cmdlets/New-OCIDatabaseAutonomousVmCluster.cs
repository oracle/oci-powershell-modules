...
{
    [Cmdlet("New", "OCIDatabaseAutonomousVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AutonomousVmCluster), typeof(Oci.DatabaseService.Responses.CreateAutonomousVmClusterResponse) })]
    public class NewOCIDatabaseAutonomousVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create an Autonomous VM cluster.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create an Autonomous VM cluster.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateAutonomousVmClusterRequest
                {
        ...
        private void HandleOutput(CreateAutonomousVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateAutonomousVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateAutonomousVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AutonomousVmCluster);
        }

        private CreateAutonomousVmClusterResponse response;
