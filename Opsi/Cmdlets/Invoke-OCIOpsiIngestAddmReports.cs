...
{
    [Cmdlet("Invoke", "OCIOpsiIngestAddmReports")]
    [OutputType(new System.Type[] { typeof(Oci.OpsiService.Models.IngestAddmReportsResponseDetails), typeof(Oci.OpsiService.Responses.IngestAddmReportsResponse) })]
    public class InvokeOCIOpsiIngestAddmReports : OCIOperationsInsightsCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Collection of addm reports for a particular database.")]
...
*Note:* Retry tokens can be invalidated before the 24 hour time limit due to conflicting operations, such as a resource being deleted or purged from the system.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new IngestAddmReportsRequest
                {
                ...
                response = client.IngestAddmReports(request).GetAwaiter().GetResult();
                WriteOutput(response, response.IngestAddmReportsResponseDetails);
        ...
        private IngestAddmReportsResponse response;
    }
}
