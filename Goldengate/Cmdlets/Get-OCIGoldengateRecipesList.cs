...
{
    [Cmdlet("Get", "OCIGoldengateRecipesList")]
    [OutputType(new System.Type[] { typeof(Oci.GoldengateService.Models.RecipeSummaryCollection), typeof(Oci.GoldengateService.Responses.ListRecipesResponse) })]
    public class GetOCIGoldengateRecipesList : OCIGoldenGateCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The pipeline's recipe type. The default value is ZERO_ETL.")]
        public System.Nullable<Oci.GoldengateService.Models.RecipeType> RecipeType { get; set; }
        ...
        public System.Nullable<Oci.GoldengateService.Requests.ListRecipesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListRecipesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.RecipeSummaryCollection, true);
                }
        ...
        {
            IEnumerable<ListRecipesResponse> DefaultRequest(ListRecipesRequest request) => Enumerable.Repeat(client.ListRecipes(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListRecipesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListRecipesResponse response;
        private delegate IEnumerable<ListRecipesResponse> RequestDelegate(ListRecipesRequest request);
