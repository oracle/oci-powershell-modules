...
{
    [Cmdlet("Get", "OCIEmailDomainsList")]
    [OutputType(new System.Type[] { typeof(Oci.EmailService.Models.EmailDomainCollection), typeof(Oci.EmailService.Responses.ListEmailDomainsResponse) })]
    public class GetOCIEmailDomainsList : OCIEmailCmdlet
    {
        ...
        public System.Nullable<Oci.EmailService.Models.EmailDomain.LifecycleStateEnum> LifecycleState { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Specifies the attribute with which to sort the email domains.

Default: `TIMECREATED`

* **TIMECREATED:** Sorts by timeCreated. * **NAME:** Sorts by name. * **ID:** Sorts by id.")]
        public System.Nullable<Oci.EmailService.Requests.ListEmailDomainsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListEmailDomainsRequest
                {
                ...
                IEnumerable<ListEmailDomainsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.EmailDomainCollection, true);
                }
        ...
        {
            IEnumerable<ListEmailDomainsResponse> DefaultRequest(ListEmailDomainsRequest request) => Enumerable.Repeat(client.ListEmailDomains(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListEmailDomainsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListEmailDomainsResponse response;
        private delegate IEnumerable<ListEmailDomainsResponse> RequestDelegate(ListEmailDomainsRequest request);
