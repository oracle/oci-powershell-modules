...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudListenersList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudListenerCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudListenersResponse) })]
    public class GetOCIDatabasemanagementCloudListenersList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudListenersRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudListenersRequest
                {
                ...
                IEnumerable<ListCloudListenersResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudListenerCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudListenersResponse> DefaultRequest(ListCloudListenersRequest request) => Enumerable.Repeat(client.ListCloudListeners(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudListenersResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudListenersResponse response;
        private delegate IEnumerable<ListCloudListenersResponse> RequestDelegate(ListCloudListenersRequest request);
