...
{
    [Cmdlet("New", "OCIDatabaseExecutionWindow", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExecutionWindow), typeof(Oci.DatabaseService.Responses.CreateExecutionWindowResponse) })]
    public class NewOCIDatabaseExecutionWindow : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create execution window.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create execution window.", ParameterSetName = Default)]
        ...
        private void HandleOutput(CreateExecutionWindowRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExecutionWindow(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExecutionWindow(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExecutionWindow);
        }

        private CreateExecutionWindowResponse response;
