...
{
    [Cmdlet("Move", "OCIDatabaseAutonomousExadataInfrastructureCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeAutonomousExadataInfrastructureCompartmentResponse) })]
    public class MoveOCIDatabaseAutonomousExadataInfrastructureCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move an Autonomous Exadata Infrastructure resource to a different compartment.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move an Autonomous Exadata Infrastructure resource to a different compartment.", ParameterSetName = Default)]
        public ChangeCompartmentDetails ChangeCompartmentDetails { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Autonomous Exadata Infrastructure  [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Autonomous Exadata Infrastructure  [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeAutonomousExadataInfrastructureCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeAutonomousExadataInfrastructureCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeAutonomousExadataInfrastructureCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeAutonomousExadataInfrastructureCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeAutonomousExadataInfrastructureCompartmentResponse response;
