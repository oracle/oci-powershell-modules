...
{
    [Cmdlet("Remove", "OCIDatabaseVirtualMachineFromExadbVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExadbVmCluster), typeof(Oci.DatabaseService.Responses.RemoveVirtualMachineFromExadbVmClusterResponse) })]
    public class RemoveOCIDatabaseVirtualMachineFromExadbVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to remove Virtual Machines from the Exadata VM cluster on Exascale Infrastructure.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to remove Virtual Machines from the Exadata VM cluster on Exascale Infrastructure.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new RemoveVirtualMachineFromExadbVmClusterRequest
                {
        ...
        private void HandleOutput(RemoveVirtualMachineFromExadbVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForRemoveVirtualMachineFromExadbVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.RemoveVirtualMachineFromExadbVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExadbVmCluster);
        }

        private RemoveVirtualMachineFromExadbVmClusterResponse response;
