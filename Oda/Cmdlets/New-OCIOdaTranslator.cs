...
{
    [Cmdlet("New", "OCIOdaTranslator")]
    [OutputType(new System.Type[] { typeof(Oci.OdaService.Models.Translator), typeof(Oci.OdaService.Responses.CreateTranslatorResponse) })]
    public class NewOCIOdaTranslator : OCIManagementCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Property values to create the new Translator.")]
...
Retry tokens expire after 24 hours, but they can become invalid before then if there are conflicting operations. For example, if an instance was deleted and purged from the system, then the service might reject a retry of the original creation request.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateTranslatorRequest
                {
                ...
                response = client.CreateTranslator(request).GetAwaiter().GetResult();
        ...
        private CreateTranslatorResponse response;
    }
}
