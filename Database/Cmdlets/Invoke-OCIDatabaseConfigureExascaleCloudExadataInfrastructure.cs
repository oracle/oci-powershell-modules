...
{
    [Cmdlet("Invoke", "OCIDatabaseConfigureExascaleCloudExadataInfrastructure", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.CloudExadataInfrastructure), typeof(Oci.DatabaseService.Responses.ConfigureExascaleCloudExadataInfrastructureResponse) })]
    public class InvokeOCIDatabaseConfigureExascaleCloudExadataInfrastructure : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The exascale config details for the Exadata infrastructure with the total storage needed.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The exascale config details for the Exadata infrastructure with the total storage needed.", ParameterSetName = Default)]
        ...
        private void HandleOutput(ConfigureExascaleCloudExadataInfrastructureRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForConfigureExascaleCloudExadataInfrastructure(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ConfigureExascaleCloudExadataInfrastructure(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.CloudExadataInfrastructure);
        }

        private ConfigureExascaleCloudExadataInfrastructureResponse response;
