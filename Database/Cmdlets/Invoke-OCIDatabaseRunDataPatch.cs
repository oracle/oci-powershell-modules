...
{
    [Cmdlet("Invoke", "OCIDatabaseRunDataPatch", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.RunDataPatchResponse) })]
    public class InvokeOCIDatabaseRunDataPatch : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to run a database datapatch.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to run a database datapatch.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new RunDataPatchRequest
                {
        ...
        private void HandleOutput(RunDataPatchRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForRunDataPatch(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.RunDataPatch(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private RunDataPatchResponse response;
