...
{
    [Cmdlet("Get", "OCIEmailDkimsList")]
    [OutputType(new System.Type[] { typeof(Oci.EmailService.Models.DkimCollection), typeof(Oci.EmailService.Responses.ListDkimsResponse) })]
    public class GetOCIEmailDkimsList : OCIEmailCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the email domain to which this DKIM belongs.")]
        ...
        public System.Nullable<Oci.EmailService.Models.Dkim.LifecycleStateEnum> LifecycleState { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Specifies the attribute with which to sort the DKIMs.

Default: `TIMECREATED`

* **TIMECREATED:** Sorts by timeCreated. * **NAME:** Sorts by name. * **ID:** Sorts by id.")]
        public System.Nullable<Oci.EmailService.Requests.ListDkimsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListDkimsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.DkimCollection, true);
                }
        ...
        {
            IEnumerable<ListDkimsResponse> DefaultRequest(ListDkimsRequest request) => Enumerable.Repeat(client.ListDkims(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListDkimsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListDkimsResponse response;
        private delegate IEnumerable<ListDkimsResponse> RequestDelegate(ListDkimsRequest request);
