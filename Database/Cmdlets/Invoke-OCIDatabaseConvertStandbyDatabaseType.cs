...
{
    [Cmdlet("Invoke", "OCIDatabaseConvertStandbyDatabaseType", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.ConvertStandbyDatabaseTypeResponse) })]
    public class InvokeOCIDatabaseConvertStandbyDatabaseType : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request parameters needed to convert Standby to snapshot standby and vice-versa.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request parameters needed to convert Standby to snapshot standby and vice-versa.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ConvertStandbyDatabaseTypeRequest
                {
        ...
        private void HandleOutput(ConvertStandbyDatabaseTypeRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForConvertStandbyDatabaseType(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ConvertStandbyDatabaseType(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private ConvertStandbyDatabaseTypeResponse response;
