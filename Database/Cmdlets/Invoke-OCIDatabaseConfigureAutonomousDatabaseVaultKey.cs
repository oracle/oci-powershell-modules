...
{
    [Cmdlet("Invoke", "OCIDatabaseConfigureAutonomousDatabaseVaultKey", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ConfigureAutonomousDatabaseVaultKeyResponse) })]
    public class InvokeOCIDatabaseConfigureAutonomousDatabaseVaultKey : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Configuration details for the Autonomous Database Vault service [key](https://docs.cloud.oracle.com/Content/KeyManagement/Concepts/keyoverview.htm#concepts).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Configuration details for the Autonomous Database Vault service [key](https://docs.cloud.oracle.com/Content/KeyManagement/Concepts/keyoverview.htm#concepts).", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ConfigureAutonomousDatabaseVaultKeyRequest
                {
        ...
        private void HandleOutput(ConfigureAutonomousDatabaseVaultKeyRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForConfigureAutonomousDatabaseVaultKey(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ConfigureAutonomousDatabaseVaultKey(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ConfigureAutonomousDatabaseVaultKeyResponse response;
