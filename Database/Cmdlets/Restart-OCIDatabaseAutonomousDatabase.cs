...
{
    [Cmdlet("Restart", "OCIDatabaseAutonomousDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AutonomousDatabase), typeof(Oci.DatabaseService.Responses.RestartAutonomousDatabaseResponse) })]
    public class RestartOCIDatabaseAutonomousDatabase : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"If provided, an online restart will be triggered.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"If provided, an online restart will be triggered.", ParameterSetName = Default)]
        ...
        private void HandleOutput(RestartAutonomousDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForRestartAutonomousDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.RestartAutonomousDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AutonomousDatabase);
        }

        private RestartAutonomousDatabaseResponse response;
