...
{
    [Cmdlet("Get", "OCIDevopsCommitDiffsList")]
    [OutputType(new System.Type[] { typeof(Oci.DevopsService.Models.DiffCollection), typeof(Oci.DevopsService.Responses.ListCommitDiffsResponse) })]
    public class GetOCIDevopsCommitDiffsList : OCIDevopsCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Unique repository identifier.")]
        public string RepositoryId { get; set; }

        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The commit or reference name to compare changes against.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCommitDiffsRequest
                {
                ...
                IEnumerable<ListCommitDiffsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.DiffCollection, true);
                }
        ...
        {
            IEnumerable<ListCommitDiffsResponse> DefaultRequest(ListCommitDiffsRequest request) => Enumerable.Repeat(client.ListCommitDiffs(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCommitDiffsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCommitDiffsResponse response;
        private delegate IEnumerable<ListCommitDiffsResponse> RequestDelegate(ListCommitDiffsRequest request);
