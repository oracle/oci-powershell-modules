...
{
    [Cmdlet("New", "OCIDatabaseExternalDatabaseConnector", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExternalDatabaseConnector), typeof(Oci.DatabaseService.Responses.CreateExternalDatabaseConnectorResponse) })]
    public class NewOCIDatabaseExternalDatabaseConnector : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a connector to an external database. This parameter also accepts subtype <Oci.DatabaseService.Models.CreateExternalMacsConnectorDetails> of type <Oci.DatabaseService.Models.CreateExternalDatabaseConnectorDetails>.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a connector to an external database. This parameter also accepts subtype <Oci.DatabaseService.Models.CreateExternalMacsConnectorDetails> of type <Oci.DatabaseService.Models.CreateExternalDatabaseConnectorDetails>.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateExternalDatabaseConnectorRequest
                {
        ...
        private void HandleOutput(CreateExternalDatabaseConnectorRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExternalDatabaseConnector(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExternalDatabaseConnector(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExternalDatabaseConnector);
        }

        private CreateExternalDatabaseConnectorResponse response;
