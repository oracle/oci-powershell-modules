...
{
    [Cmdlet("Enable", "OCIDatabaseExternalNonContainerDatabaseStackMonitoring", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.EnableExternalNonContainerDatabaseStackMonitoringResponse) })]
    public class EnableOCIDatabaseExternalNonContainerDatabaseStackMonitoring : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to enable Stack Monitoring on the external non-container database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to enable Stack Monitoring on the external non-container database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableExternalNonContainerDatabaseStackMonitoringRequest
                {
        ...
        private void HandleOutput(EnableExternalNonContainerDatabaseStackMonitoringRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableExternalNonContainerDatabaseStackMonitoring(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableExternalNonContainerDatabaseStackMonitoring(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private EnableExternalNonContainerDatabaseStackMonitoringResponse response;
