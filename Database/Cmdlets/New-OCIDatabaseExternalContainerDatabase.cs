...
{
    [Cmdlet("New", "OCIDatabaseExternalContainerDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExternalContainerDatabase), typeof(Oci.DatabaseService.Responses.CreateExternalContainerDatabaseResponse) })]
    public class NewOCIDatabaseExternalContainerDatabase : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new external container database resource.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new external container database resource.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateExternalContainerDatabaseRequest
                {
        ...
        private void HandleOutput(CreateExternalContainerDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExternalContainerDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExternalContainerDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExternalContainerDatabase);
        }

        private CreateExternalContainerDatabaseResponse response;
