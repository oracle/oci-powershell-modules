...
{
    [Cmdlet("Get", "OCIDnsTsigKey", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DnsService.Models.TsigKey), typeof(Oci.DnsService.Responses.GetTsigKeyResponse) })]
    public class GetOCIDnsTsigKey : OCIDnsCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The OCID of the target TSIG key.", ParameterSetName = LifecycleStateParamSet)]
        ...
        private void HandleOutput(GetTsigKeyRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForTsigKey(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetTsigKey(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.TsigKey);
        }

        private GetTsigKeyResponse response;
