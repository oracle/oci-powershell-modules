...
{
    [Cmdlet("Invoke", "OCIVirtualNetworkPatchSubnet", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.PatchSubnetResponse) })]
    public class InvokeOCIVirtualNetworkPatchSubnet : OCIVirtualNetworkCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for patching a subnet.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for patching a subnet.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new PatchSubnetRequest
                {
                    SubnetId = SubnetId,
        ...
        private void HandleOutput(PatchSubnetRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForPatchSubnet(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.PatchSubnet(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private PatchSubnetResponse response;
