...
{
    [Cmdlet("Stop", "OCIDatabaseExecutionWindow", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExecutionWindow), typeof(Oci.DatabaseService.Responses.CancelExecutionWindowResponse) })]
    public class StopOCIDatabaseExecutionWindow : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to cancel the in progress maintenance activity under this execution window.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to cancel the in progress maintenance activity under this execution window.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CancelExecutionWindowRequest
                {
        ...
        private void HandleOutput(CancelExecutionWindowRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCancelExecutionWindow(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CancelExecutionWindow(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExecutionWindow);
        }

        private CancelExecutionWindowResponse response;
