...
{
    [Cmdlet("Move", "OCIDatabaseCloudExadataInfrastructureCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeCloudExadataInfrastructureCompartmentResponse) })]
    public class MoveOCIDatabaseCloudExadataInfrastructureCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move cloud Exadata infrastructure resource to a different compartment.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move cloud Exadata infrastructure resource to a different compartment.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeCloudExadataInfrastructureCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeCloudExadataInfrastructureCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeCloudExadataInfrastructureCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeCloudExadataInfrastructureCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeCloudExadataInfrastructureCompartmentResponse response;
