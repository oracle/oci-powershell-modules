...
{
    [Cmdlet("Move", "OCIDatabaseExascaleDbStorageVaultCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeExascaleDbStorageVaultCompartmentResponse) })]
    public class MoveOCIDatabaseExascaleDbStorageVaultCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move Exadata Database Storage Vault to a different compartment", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move Exadata Database Storage Vault to a different compartment", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeExascaleDbStorageVaultCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeExascaleDbStorageVaultCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeExascaleDbStorageVaultCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeExascaleDbStorageVaultCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeExascaleDbStorageVaultCompartmentResponse response;
