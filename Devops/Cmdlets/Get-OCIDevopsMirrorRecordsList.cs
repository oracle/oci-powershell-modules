...
{
    [Cmdlet("Get", "OCIDevopsMirrorRecordsList")]
    ...
    public class GetOCIDevopsMirrorRecordsList : OCIDevopsCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListMirrorRecordsRequest
                {
                ...
                IEnumerable<ListMirrorRecordsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.RepositoryMirrorRecordCollection, true);
                }
        ...
        {
            IEnumerable<ListMirrorRecordsResponse> DefaultRequest(ListMirrorRecordsRequest request) => Enumerable.Repeat(client.ListMirrorRecords(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListMirrorRecordsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListMirrorRecordsResponse response;
        private delegate IEnumerable<ListMirrorRecordsResponse> RequestDelegate(ListMirrorRecordsRequest request);
