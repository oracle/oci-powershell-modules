...
{
    [Cmdlet("New", "OCIOdaPrivateEndpointAttachment")]
    [OutputType(new System.Type[] { typeof(Oci.OdaService.Models.OdaPrivateEndpointAttachment), typeof(Oci.OdaService.Responses.CreateOdaPrivateEndpointAttachmentResponse) })]
    public class NewOCIOdaPrivateEndpointAttachment : OCIManagementCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for the new ODA Private Endpoint Attachment.")]
...
Retry tokens expire after 24 hours, but they can become invalid before then if there are conflicting operations. For example, if an instance was deleted and purged from the system, then the service might reject a retry of the original creation request.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateOdaPrivateEndpointAttachmentRequest
                {
                ...
                response = client.CreateOdaPrivateEndpointAttachment(request).GetAwaiter().GetResult();
        ...
        private CreateOdaPrivateEndpointAttachmentResponse response;
    }
}
