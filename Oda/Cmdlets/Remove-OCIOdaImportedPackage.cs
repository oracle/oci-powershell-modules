...
{
    [Cmdlet("Remove", "OCIOdaImportedPackage", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.High)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.OdaService.Responses.DeleteImportedPackageResponse) })]
    public class RemoveOCIOdaImportedPackage : OCIOdapackageCmdlet
    {
...
Retry tokens expire after 24 hours, but they can become invalid before then if there are conflicting operations. For example, if an instance was deleted and purged from the system, then the service might reject a retry of the original creation request.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (!ConfirmDelete("OCIOdaImportedPackage", "Remove"))
            {
               return;
            }
            ...
            try
            {
                request = new DeleteImportedPackageRequest
                {
                ...
                response = client.DeleteImportedPackage(request).GetAwaiter().GetResult();
        ...
        private DeleteImportedPackageResponse response;
    }
}
