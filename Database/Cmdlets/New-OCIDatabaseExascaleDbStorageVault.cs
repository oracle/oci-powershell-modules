...
{
    [Cmdlet("New", "OCIDatabaseExascaleDbStorageVault", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExascaleDbStorageVault), typeof(Oci.DatabaseService.Responses.CreateExascaleDbStorageVaultResponse) })]
    public class NewOCIDatabaseExascaleDbStorageVault : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a Exadata Database Storage Vault.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a Exadata Database Storage Vault.", ParameterSetName = Default)]
        ...
        private void HandleOutput(CreateExascaleDbStorageVaultRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExascaleDbStorageVault(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExascaleDbStorageVault(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExascaleDbStorageVault);
        }

        private CreateExascaleDbStorageVaultResponse response;
