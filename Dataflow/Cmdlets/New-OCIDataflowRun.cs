...
{
    [Cmdlet("New", "OCIDataflowRun")]
    [OutputType(new System.Type[] { typeof(Oci.DataflowService.Models.Run), typeof(Oci.DataflowService.Responses.CreateRunResponse) })]
    public class NewOCIDataflowRun : OCIDataFlowCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for creating a run of an application.")]
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"(Optional header param, required for Resource Principal version 3.0+) Parent resource control plane endpoint used to exchange for upper level resource principal token.")]
        ...
        private CreateRunResponse response;
    }
}
