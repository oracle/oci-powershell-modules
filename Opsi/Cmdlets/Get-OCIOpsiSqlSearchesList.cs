...
{
    [Cmdlet("Get", "OCIOpsiSqlSearchesList")]
    [OutputType(new System.Type[] { typeof(Oci.OpsiService.Models.SqlSearchCollection), typeof(Oci.OpsiService.Responses.ListSqlSearchesResponse) })]
    public class GetOCIOpsiSqlSearchesList : OCIOperationsInsightsCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListSqlSearchesRequest
                {
                ...
                IEnumerable<ListSqlSearchesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.SqlSearchCollection, true);
                }
        ...
        {
            IEnumerable<ListSqlSearchesResponse> DefaultRequest(ListSqlSearchesRequest request) => Enumerable.Repeat(client.ListSqlSearches(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListSqlSearchesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListSqlSearchesResponse response;
        private delegate IEnumerable<ListSqlSearchesResponse> RequestDelegate(ListSqlSearchesRequest request);
