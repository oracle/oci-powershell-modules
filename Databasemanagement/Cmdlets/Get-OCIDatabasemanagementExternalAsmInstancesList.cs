...
{
    [Cmdlet("Get", "OCIDatabasemanagementExternalAsmInstancesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.ExternalAsmInstanceCollection), typeof(Oci.DatabasemanagementService.Responses.ListExternalAsmInstancesResponse) })]
    public class GetOCIDatabasemanagementExternalAsmInstancesList : OCIDbManagementCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm) of the external ASM.")]
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListExternalAsmInstancesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListExternalAsmInstancesRequest
                {
                ...
                IEnumerable<ListExternalAsmInstancesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ExternalAsmInstanceCollection, true);
                }
        ...
        {
            IEnumerable<ListExternalAsmInstancesResponse> DefaultRequest(ListExternalAsmInstancesRequest request) => Enumerable.Repeat(client.ListExternalAsmInstances(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListExternalAsmInstancesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListExternalAsmInstancesResponse response;
        private delegate IEnumerable<ListExternalAsmInstancesResponse> RequestDelegate(ListExternalAsmInstancesRequest request);
