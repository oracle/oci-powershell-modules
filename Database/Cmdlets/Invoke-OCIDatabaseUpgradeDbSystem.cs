...
{
    [Cmdlet("Invoke", "OCIDatabaseUpgradeDbSystem", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbSystem), typeof(Oci.DatabaseService.Responses.UpgradeDbSystemResponse) })]
    public class InvokeOCIDatabaseUpgradeDbSystem : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to perform an upgrade of the operating system and the Oracle Grid Infrastructure (GI) of the DB system.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to perform an upgrade of the operating system and the Oracle Grid Infrastructure (GI) of the DB system.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpgradeDbSystemRequest
                {
        ...
        private void HandleOutput(UpgradeDbSystemRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpgradeDbSystem(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpgradeDbSystem(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbSystem);
        }

        private UpgradeDbSystemResponse response;
