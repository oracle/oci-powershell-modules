...
{
    [Cmdlet("New", "OCIOdaImportedPackage")]
    [OutputType(new System.Type[] { typeof(Oci.OdaService.Models.ImportedPackage), typeof(Oci.OdaService.Responses.CreateImportedPackageResponse) })]
    public class NewOCIOdaImportedPackage : OCIOdapackageCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Parameter values required to import the package.")]
...
Retry tokens expire after 24 hours, but they can become invalid before then if there are conflicting operations. For example, if an instance was deleted and purged from the system, then the service might reject a retry of the original creation request.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateImportedPackageRequest
                {
                ...
                response = client.CreateImportedPackage(request).GetAwaiter().GetResult();
        ...
        private CreateImportedPackageResponse response;
    }
}
