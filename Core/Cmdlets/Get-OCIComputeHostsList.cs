...
{
    [Cmdlet("Get", "OCIComputeHostsList")]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.ComputeHostCollection), typeof(Oci.CoreService.Responses.ListComputeHostsResponse) })]
    public class GetOCIComputeHostsList : OCIComputeCmdlet
    {
...
Example: `Uocm:PHX-AD-1`")]
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the compute host network resoruce. - Customer-unique HPC island ID - Customer-unique network block ID - Customer-unique local block ID")]
...
**Note:** In general, some ""List"" operations (for example, `ListInstances`) let you optionally filter by availability domain if the scope of the resource type is within a single availability domain. If you call one of these ""List"" operations without specifying an availability domain, the resources are grouped by availability domain, then sorted.")]
        public System.Nullable<Oci.CoreService.Requests.ListComputeHostsRequest.SortByEnum> SortBy { get; set; }
        ...
        public System.Nullable<Oci.CoreService.Requests.ListComputeHostsRequest.SortOrderEnum> SortOrder { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to return only ComputeHostSummary resources that match the given Compute Host lifecycle State OCID exactly.")]
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the compute host group.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListComputeHostsRequest
                {
                ...
                {
                    response = item;
                    WriteOutput(response, response.ComputeHostCollection, true);
                }
        ...
        {
            IEnumerable<ListComputeHostsResponse> DefaultRequest(ListComputeHostsRequest request) => Enumerable.Repeat(client.ListComputeHosts(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListComputeHostsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListComputeHostsResponse response;
        private delegate IEnumerable<ListComputeHostsResponse> RequestDelegate(ListComputeHostsRequest request);
