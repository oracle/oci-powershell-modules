...
{
    [Cmdlet("Invoke", "OCIDatabaseConvertToPdb", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.ConvertToPdbResponse) })]
    public class InvokeOCIDatabaseConvertToPdb : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to convert a non-container database to a pluggable database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to convert a non-container database to a pluggable database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ConvertToPdbRequest
                {
        ...
        private void HandleOutput(ConvertToPdbRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForConvertToPdb(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ConvertToPdb(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private ConvertToPdbResponse response;
