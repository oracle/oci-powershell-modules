...
{
    [Cmdlet("Get", "OCINetworkloadbalancerBackendsList")]
    [OutputType(new System.Type[] { typeof(Oci.NetworkloadbalancerService.Models.BackendCollection), typeof(Oci.NetworkloadbalancerService.Responses.ListBackendsResponse) })]
    public class GetOCINetworkloadbalancerBackendsList : OCINetworkLoadBalancerCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The name of the backend set associated with the backend servers.
...
Example: `example-etag`")]
        ...
        public System.Nullable<Oci.NetworkloadbalancerService.Requests.ListBackendsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListBackendsRequest
                {
                ...
                IEnumerable<ListBackendsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.BackendCollection, true);
                }
        ...
        {
            IEnumerable<ListBackendsResponse> DefaultRequest(ListBackendsRequest request) => Enumerable.Repeat(client.ListBackends(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListBackendsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListBackendsResponse response;
        private delegate IEnumerable<ListBackendsResponse> RequestDelegate(ListBackendsRequest request);
