...
{
    [Cmdlet("Get", "OCIDatabasemanagementManagedMySqlDatabasesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.ManagedMySqlDatabaseCollection), typeof(Oci.DatabasemanagementService.Responses.ListManagedMySqlDatabasesResponse) })]
    public class GetOCIDatabasemanagementManagedMySqlDatabasesList : OCIManagedMySqlDatabasesCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListManagedMySqlDatabasesRequest
                {
                ...
                IEnumerable<ListManagedMySqlDatabasesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ManagedMySqlDatabaseCollection, true);
                }
        ...
        {
            IEnumerable<ListManagedMySqlDatabasesResponse> DefaultRequest(ListManagedMySqlDatabasesRequest request) => Enumerable.Repeat(client.ListManagedMySqlDatabases(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListManagedMySqlDatabasesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListManagedMySqlDatabasesResponse response;
        private delegate IEnumerable<ListManagedMySqlDatabasesResponse> RequestDelegate(ListManagedMySqlDatabasesRequest request);
