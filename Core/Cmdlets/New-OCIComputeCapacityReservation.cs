...
{
    [Cmdlet("New", "OCIComputeCapacityReservation", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.ComputeCapacityReservation), typeof(Oci.CoreService.Responses.CreateComputeCapacityReservationResponse) })]
    public class NewOCIComputeCapacityReservation : OCIComputeCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for creating a new compute capacity reservation.

**Caution:** Avoid using any confidential information when you use the API to supply string values.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for creating a new compute capacity reservation.

**Caution:** Avoid using any confidential information when you use the API to supply string values.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateComputeCapacityReservationRequest
                {
        ...
        private void HandleOutput(CreateComputeCapacityReservationRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateComputeCapacityReservation(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateComputeCapacityReservation(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ComputeCapacityReservation);
        }

        private CreateComputeCapacityReservationResponse response;
