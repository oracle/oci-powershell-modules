...
{
    [Cmdlet("Update", "OCIDnsTsigKey", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DnsService.Models.TsigKey), typeof(Oci.DnsService.Responses.UpdateTsigKeyResponse) })]
    public class UpdateOCIDnsTsigKey : OCIDnsCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"New data for the TSIG key.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"New data for the TSIG key.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpdateTsigKeyRequest
                {
                    TsigKeyId = TsigKeyId,
        ...
        private void HandleOutput(UpdateTsigKeyRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateTsigKey(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateTsigKey(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.TsigKey);
        }

        private UpdateTsigKeyResponse response;
