...
{
    [Cmdlet("New", "OCIDatabaseSchedulingPlan", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.SchedulingPlan), typeof(Oci.DatabaseService.Responses.CreateSchedulingPlanResponse) })]
    public class NewOCIDatabaseSchedulingPlan : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create Scheduling Plan.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create Scheduling Plan.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateSchedulingPlanRequest
                {
        ...
        private void HandleOutput(CreateSchedulingPlanRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateSchedulingPlan(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateSchedulingPlan(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.SchedulingPlan);
        }

        private CreateSchedulingPlanResponse response;
