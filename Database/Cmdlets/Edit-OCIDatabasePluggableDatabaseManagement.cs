...
{
    [Cmdlet("Edit", "OCIDatabasePluggableDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabase), typeof(Oci.DatabaseService.Responses.ModifyPluggableDatabaseManagementResponse) })]
    public class EditOCIDatabasePluggableDatabaseManagement : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The data to update one or more attributes of the Database Management Service for the pluggable database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The data to update one or more attributes of the Database Management Service for the pluggable database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ModifyPluggableDatabaseManagementRequest
                {
        ...
        private void HandleOutput(ModifyPluggableDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForModifyPluggableDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ModifyPluggableDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabase);
        }

        private ModifyPluggableDatabaseManagementResponse response;
