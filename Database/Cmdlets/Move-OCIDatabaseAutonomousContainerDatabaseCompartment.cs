...
{
    [Cmdlet("Move", "OCIDatabaseAutonomousContainerDatabaseCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeAutonomousContainerDatabaseCompartmentResponse) })]
    public class MoveOCIDatabaseAutonomousContainerDatabaseCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move Autonomous Container Database to a different compartment", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move Autonomous Container Database to a different compartment", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeAutonomousContainerDatabaseCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeAutonomousContainerDatabaseCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeAutonomousContainerDatabaseCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeAutonomousContainerDatabaseCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeAutonomousContainerDatabaseCompartmentResponse response;
