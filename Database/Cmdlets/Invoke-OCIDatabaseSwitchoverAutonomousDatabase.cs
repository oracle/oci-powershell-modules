...
{
    [Cmdlet("Invoke", "OCIDatabaseSwitchoverAutonomousDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AutonomousDatabase), typeof(Oci.DatabaseService.Responses.SwitchoverAutonomousDatabaseResponse) })]
    public class InvokeOCIDatabaseSwitchoverAutonomousDatabase : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The database OCID(/Content/General/Concepts/identifiers.htm) of the Disaster Recovery peer (source Primary) database, which is located in a different (remote) region from the current peer database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The database OCID(/Content/General/Concepts/identifiers.htm) of the Disaster Recovery peer (source Primary) database, which is located in a different (remote) region from the current peer database.", ParameterSetName = Default)]
        ...
        private void HandleOutput(SwitchoverAutonomousDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForSwitchoverAutonomousDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.SwitchoverAutonomousDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AutonomousDatabase);
        }

        private SwitchoverAutonomousDatabaseResponse response;
