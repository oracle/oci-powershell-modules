...
{
    [Cmdlet("New", "OCIDatabasePluggableDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabase), typeof(Oci.DatabaseService.Responses.CreatePluggableDatabaseResponse) })]
    public class NewOCIDatabasePluggableDatabase : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create pluggable database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create pluggable database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreatePluggableDatabaseRequest
                {
        ...
        private void HandleOutput(CreatePluggableDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreatePluggableDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreatePluggableDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabase);
        }

        private CreatePluggableDatabaseResponse response;
