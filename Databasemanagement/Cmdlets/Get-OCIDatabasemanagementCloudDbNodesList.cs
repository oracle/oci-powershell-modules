...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudDbNodesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudDbNodeCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudDbNodesResponse) })]
    public class GetOCIDatabasemanagementCloudDbNodesList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudDbNodesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudDbNodesRequest
                {
                ...
                IEnumerable<ListCloudDbNodesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudDbNodeCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudDbNodesResponse> DefaultRequest(ListCloudDbNodesRequest request) => Enumerable.Repeat(client.ListCloudDbNodes(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudDbNodesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudDbNodesResponse response;
        private delegate IEnumerable<ListCloudDbNodesResponse> RequestDelegate(ListCloudDbNodesRequest request);
