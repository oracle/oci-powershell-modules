...
{
    [Cmdlet("Move", "OCIDatabaseExecutionActionMember", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExecutionAction), typeof(Oci.DatabaseService.Responses.MoveExecutionActionMemberResponse) })]
    public class MoveOCIDatabaseExecutionActionMember : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The execution action [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The execution action [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        public string ExecutionActionId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move an execution action member to this execution action resource from another.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move an execution action member to this execution action resource from another.", ParameterSetName = Default)]
        ...
        private void HandleOutput(MoveExecutionActionMemberRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForMoveExecutionActionMember(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.MoveExecutionActionMember(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExecutionAction);
        }

        private MoveExecutionActionMemberResponse response;
