...
{
    [Cmdlet("Move", "OCIDatabaseDisasterRecoveryConfiguration", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AutonomousDatabase), typeof(Oci.DatabaseService.Responses.ChangeDisasterRecoveryConfigurationResponse) })]
    public class MoveOCIDatabaseDisasterRecoveryConfiguration : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the cross-region disaster recovery (DR) details of the standby Autonomous AI Database Serverless database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the cross-region disaster recovery (DR) details of the standby Autonomous AI Database Serverless database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeDisasterRecoveryConfigurationRequest
                {
        ...
        private void HandleOutput(ChangeDisasterRecoveryConfigurationRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeDisasterRecoveryConfiguration(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeDisasterRecoveryConfiguration(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AutonomousDatabase);
        }

        private ChangeDisasterRecoveryConfigurationResponse response;
