...
{
    [Cmdlet("Get", "OCIMysqlReplica", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.MysqlService.Models.Replica), typeof(Oci.MysqlService.Responses.GetReplicaResponse) })]
    public class GetOCIMysqlReplica : OCIReplicasCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Replica [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = LifecycleStateParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Replica [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        ...
        public Oci.MysqlService.Models.Replica.LifecycleStateEnum[] WaitForLifecycleState { get; set; }
        ...
        private void HandleOutput(GetReplicaRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForReplica(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetReplica(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Replica);
        }

        private GetReplicaResponse response;
