...
{
    [Cmdlet("Move", "OCIDnsViewCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DnsService.Responses.ChangeViewCompartmentResponse) })]
    public class MoveOCIDnsViewCompartment : OCIDnsCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The OCID of the target view.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The OCID of the target view.", ParameterSetName = Default)]
        public string ViewId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for moving a view into a different compartment.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for moving a view into a different compartment.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeViewCompartmentRequest
                {
                    ViewId = ViewId,
        ...
        private void HandleOutput(ChangeViewCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeViewCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeViewCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeViewCompartmentResponse response;
