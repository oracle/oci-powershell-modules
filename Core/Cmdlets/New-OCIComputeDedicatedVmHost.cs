...
{
    [Cmdlet("New", "OCIComputeDedicatedVmHost", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.DedicatedVmHost), typeof(Oci.CoreService.Responses.CreateDedicatedVmHostResponse) })]
    public class NewOCIComputeDedicatedVmHost : OCIComputeCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The details for creating a new dedicated virtual machine host.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The details for creating a new dedicated virtual machine host.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateDedicatedVmHostRequest
                {
        ...
        private void HandleOutput(CreateDedicatedVmHostRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateDedicatedVmHost(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateDedicatedVmHost(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DedicatedVmHost);
        }

        private CreateDedicatedVmHostResponse response;
