...
{
    [Cmdlet("Invoke", "OCIVirtualNetworkBulkDetachPrivateIps", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.BulkDetachPrivateIpsResponse) })]
    public class InvokeOCIVirtualNetworkBulkDetachPrivateIps : OCIVirtualNetworkCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The secondary IPv4 addresses to detach.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The secondary IPv4 addresses to detach.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new BulkDetachPrivateIpsRequest
                {
        ...
        private void HandleOutput(BulkDetachPrivateIpsRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForBulkDetachPrivateIps(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.BulkDetachPrivateIps(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private BulkDetachPrivateIpsResponse response;
