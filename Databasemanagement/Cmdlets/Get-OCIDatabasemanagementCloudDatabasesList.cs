...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudDatabasesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudDatabaseCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudDatabasesResponse) })]
    public class GetOCIDatabasemanagementCloudDatabasesList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudDatabasesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudDatabasesRequest
                {
                ...
                IEnumerable<ListCloudDatabasesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudDatabaseCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudDatabasesResponse> DefaultRequest(ListCloudDatabasesRequest request) => Enumerable.Repeat(client.ListCloudDatabases(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudDatabasesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudDatabasesResponse response;
        private delegate IEnumerable<ListCloudDatabasesResponse> RequestDelegate(ListCloudDatabasesRequest request);
