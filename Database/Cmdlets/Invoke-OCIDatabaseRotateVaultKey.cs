...
{
    [Cmdlet("Invoke", "OCIDatabaseRotateVaultKey", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.RotateVaultKeyResponse) })]
    public class InvokeOCIDatabaseRotateVaultKey : OCIDatabaseCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new RotateVaultKeyRequest
                {
        ...
        private void HandleOutput(RotateVaultKeyRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForRotateVaultKey(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.RotateVaultKey(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private RotateVaultKeyResponse response;
