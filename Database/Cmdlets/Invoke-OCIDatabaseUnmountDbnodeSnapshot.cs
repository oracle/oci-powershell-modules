...
{
    [Cmdlet("Invoke", "OCIDatabaseUnmountDbnodeSnapshot", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbnodeSnapshot), typeof(Oci.DatabaseService.Responses.UnmountDbnodeSnapshotResponse) })]
    public class InvokeOCIDatabaseUnmountDbnodeSnapshot : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to unmount the snapshot for the provided Exadata Database Node.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to unmount the snapshot for the provided Exadata Database Node.", ParameterSetName = Default)]
        public UnmountDbnodeSnapshotDetails UnmountDbnodeSnapshotDetails { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Exadata Database Node Snapshot [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Exadata Database Node Snapshot [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UnmountDbnodeSnapshotRequest
                {
        ...
        private void HandleOutput(UnmountDbnodeSnapshotRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUnmountDbnodeSnapshot(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UnmountDbnodeSnapshot(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbnodeSnapshot);
        }

        private UnmountDbnodeSnapshotResponse response;
