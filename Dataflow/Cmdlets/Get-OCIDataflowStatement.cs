...
{
    [Cmdlet("Get", "OCIDataflowStatement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DataflowService.Models.Statement), typeof(Oci.DataflowService.Responses.GetStatementResponse) })]
    public class GetOCIDataflowStatement : OCIDataFlowCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The unique ID for the statement.", ParameterSetName = LifecycleStateParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The unique ID for the statement.", ParameterSetName = Default)]
        ...
        public Oci.DataflowService.Models.StatementLifecycleState[] WaitForLifecycleState { get; set; }
        ...
        private void HandleOutput(GetStatementRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForStatement(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetStatement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Statement);
        }

        private GetStatementResponse response;
