...
{
    [Cmdlet("Get", "OCIOdaSkillsList")]
    [OutputType(new System.Type[] { typeof(Oci.OdaService.Models.SkillCollection), typeof(Oci.OdaService.Responses.ListSkillsResponse) })]
    public class GetOCIOdaSkillsList : OCIManagementCmdlet
    {
...
Example: `MToxMA==`")]
        ...
        public System.Nullable<Oci.OdaService.Requests.ListSkillsRequest.SortOrderEnum> SortOrder { get; set; }
...
The default sort order for `timeCreated` and `timeUpdated` is descending. For all other sort fields the default sort order is ascending.")]
        public System.Nullable<Oci.OdaService.Requests.ListSkillsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListSkillsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.SkillCollection, true);
                }
        ...
        {
            IEnumerable<ListSkillsResponse> DefaultRequest(ListSkillsRequest request) => Enumerable.Repeat(client.ListSkills(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListSkillsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListSkillsResponse response;
        private delegate IEnumerable<ListSkillsResponse> RequestDelegate(ListSkillsRequest request);
