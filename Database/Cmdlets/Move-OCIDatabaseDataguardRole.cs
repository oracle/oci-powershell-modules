...
{
    [Cmdlet("Move", "OCIDatabaseDataguardRole", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AutonomousContainerDatabase), typeof(Oci.DatabaseService.Responses.ChangeDataguardRoleResponse) })]
    public class MoveOCIDatabaseDataguardRole : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to Change the Autonomous Container Database Dataguard role.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to Change the Autonomous Container Database Dataguard role.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeDataguardRoleRequest
                {
        ...
        private void HandleOutput(ChangeDataguardRoleRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeDataguardRole(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeDataguardRole(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AutonomousContainerDatabase);
        }

        private ChangeDataguardRoleResponse response;
