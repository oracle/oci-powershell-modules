...
{
    [Cmdlet("New", "OCIDatabaseConsoleHistory", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ConsoleHistory), typeof(Oci.DatabaseService.Responses.CreateConsoleHistoryResponse) })]
    public class NewOCIDatabaseConsoleHistory : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request object for creating a console history.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request object for creating a console history.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateConsoleHistoryRequest
                {
        ...
        private void HandleOutput(CreateConsoleHistoryRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateConsoleHistory(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateConsoleHistory(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ConsoleHistory);
        }

        private CreateConsoleHistoryResponse response;
