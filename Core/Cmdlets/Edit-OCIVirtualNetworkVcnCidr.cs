...
{
    [Cmdlet("Edit", "OCIVirtualNetworkVcnCidr", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.ModifyVcnCidrResponse) })]
    public class EditOCIVirtualNetworkVcnCidr : OCIVirtualNetworkCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for updating a VCN CIDR.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for updating a VCN CIDR.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ModifyVcnCidrRequest
                {
                    VcnId = VcnId,
        ...
        private void HandleOutput(ModifyVcnCidrRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForModifyVcnCidr(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ModifyVcnCidr(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ModifyVcnCidrResponse response;
