...
{
    [Cmdlet("Get", "OCIApiaccesscontrolApiMetadataList")]
    [OutputType(new System.Type[] { typeof(Oci.ApiaccesscontrolService.Models.ApiMetadataCollection), typeof(Oci.ApiaccesscontrolService.Responses.ListApiMetadataResponse) })]
    public class GetOCIApiaccesscontrolApiMetadataList : OCIApiMetadataCmdlet
    {
        ...
        public System.Nullable<Oci.ApiaccesscontrolService.Requests.ListApiMetadataRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListApiMetadataRequest
                {
                ...
                IEnumerable<ListApiMetadataResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ApiMetadataCollection, true);
                }
        ...
        {
            IEnumerable<ListApiMetadataResponse> DefaultRequest(ListApiMetadataRequest request) => Enumerable.Repeat(client.ListApiMetadata(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListApiMetadataResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListApiMetadataResponse response;
        private delegate IEnumerable<ListApiMetadataResponse> RequestDelegate(ListApiMetadataRequest request);
