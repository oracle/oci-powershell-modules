...
{
    [Cmdlet("Get", "OCIOcvpDatastoresList")]
    [OutputType(new System.Type[] { typeof(Oci.OcvpService.Models.DatastoreCollection), typeof(Oci.OcvpService.Responses.ListDatastoresResponse) })]
    public class GetOCIOcvpDatastoresList : OCIDatastoreCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the Datastore.")]
...
**Note:** In general, some ""List"" operations (for example, `ListInstances`) let you optionally filter by availability domain if the scope of the resource type is within a single availability domain. If you call one of these ""List"" operations without specifying an availability domain, the resources are grouped by availability domain, then sorted.")]
        public System.Nullable<Oci.OcvpService.Requests.ListDatastoresRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListDatastoresResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.DatastoreCollection, true);
                }
        ...
        {
            IEnumerable<ListDatastoresResponse> DefaultRequest(ListDatastoresRequest request) => Enumerable.Repeat(client.ListDatastores(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListDatastoresResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListDatastoresResponse response;
        private delegate IEnumerable<ListDatastoresResponse> RequestDelegate(ListDatastoresRequest request);
