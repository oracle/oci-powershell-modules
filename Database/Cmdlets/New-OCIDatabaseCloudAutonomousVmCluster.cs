...
{
    [Cmdlet("New", "OCIDatabaseCloudAutonomousVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.CloudAutonomousVmCluster), typeof(Oci.DatabaseService.Responses.CreateCloudAutonomousVmClusterResponse) })]
    public class NewOCIDatabaseCloudAutonomousVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a cloud Autonomous VM cluster.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a cloud Autonomous VM cluster.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateCloudAutonomousVmClusterRequest
                {
        ...
        private void HandleOutput(CreateCloudAutonomousVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateCloudAutonomousVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateCloudAutonomousVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.CloudAutonomousVmCluster);
        }

        private CreateCloudAutonomousVmClusterResponse response;
