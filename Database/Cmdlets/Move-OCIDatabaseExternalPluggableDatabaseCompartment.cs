...
{
    [Cmdlet("Move", "OCIDatabaseExternalPluggableDatabaseCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeExternalPluggableDatabaseCompartmentResponse) })]
    public class MoveOCIDatabaseExternalPluggableDatabaseCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move the CreateExternalPluggableDatabaseDetails resource to a different compartment.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move the CreateExternalPluggableDatabaseDetails resource to a different compartment.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeExternalPluggableDatabaseCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeExternalPluggableDatabaseCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeExternalPluggableDatabaseCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeExternalPluggableDatabaseCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeExternalPluggableDatabaseCompartmentResponse response;
