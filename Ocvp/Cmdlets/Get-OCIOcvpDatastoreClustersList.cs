...
{
    [Cmdlet("Get", "OCIOcvpDatastoreClustersList")]
    [OutputType(new System.Type[] { typeof(Oci.OcvpService.Models.DatastoreClusterCollection), typeof(Oci.OcvpService.Responses.ListDatastoreClustersResponse) })]
    public class GetOCIOcvpDatastoreClustersList : OCIDatastoreClusterCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the Datastore Cluster.")]
        public string DatastoreClusterId { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the SDDC Cluster.")]
...
**Note:** In general, some ""List"" operations (for example, `ListInstances`) let you optionally filter by availability domain if the scope of the resource type is within a single availability domain. If you call one of these ""List"" operations without specifying an availability domain, the resources are grouped by availability domain, then sorted.")]
        public System.Nullable<Oci.OcvpService.Requests.ListDatastoreClustersRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListDatastoreClustersRequest
                {
                ...
                IEnumerable<ListDatastoreClustersResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.DatastoreClusterCollection, true);
                }
        ...
        {
            IEnumerable<ListDatastoreClustersResponse> DefaultRequest(ListDatastoreClustersRequest request) => Enumerable.Repeat(client.ListDatastoreClusters(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListDatastoreClustersResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListDatastoreClustersResponse response;
        private delegate IEnumerable<ListDatastoreClustersResponse> RequestDelegate(ListDatastoreClustersRequest request);
