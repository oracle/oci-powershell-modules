...
{
    [Cmdlet("Enable", "OCIDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.EnableDatabaseManagementResponse) })]
    public class EnableOCIDatabaseManagement : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management service for an Oracle Database located in Oracle Cloud Infrastructure.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management service for an Oracle Database located in Oracle Cloud Infrastructure.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableDatabaseManagementRequest
                {
        ...
        private void HandleOutput(EnableDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private EnableDatabaseManagementResponse response;
