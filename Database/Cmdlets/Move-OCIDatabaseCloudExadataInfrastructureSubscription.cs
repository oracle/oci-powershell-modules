...
{
    [Cmdlet("Move", "OCIDatabaseCloudExadataInfrastructureSubscription", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeCloudExadataInfrastructureSubscriptionResponse) })]
    public class MoveOCIDatabaseCloudExadataInfrastructureSubscription : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a cloud Exadata infrastructure with a different subscription.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a cloud Exadata infrastructure with a different subscription.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeCloudExadataInfrastructureSubscriptionRequest
                {
        ...
        private void HandleOutput(ChangeCloudExadataInfrastructureSubscriptionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeCloudExadataInfrastructureSubscription(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeCloudExadataInfrastructureSubscription(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeCloudExadataInfrastructureSubscriptionResponse response;
