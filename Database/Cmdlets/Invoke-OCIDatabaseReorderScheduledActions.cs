...
{
    [Cmdlet("Invoke", "OCIDatabaseReorderScheduledActions", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.SchedulingPlan), typeof(Oci.DatabaseService.Responses.ReorderScheduledActionsResponse) })]
    public class InvokeOCIDatabaseReorderScheduledActions : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to re-order Scheduled Action resources.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to re-order Scheduled Action resources.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ReorderScheduledActionsRequest
                {
        ...
        private void HandleOutput(ReorderScheduledActionsRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForReorderScheduledActions(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ReorderScheduledActions(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.SchedulingPlan);
        }

        private ReorderScheduledActionsResponse response;
