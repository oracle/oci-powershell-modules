...
{
    [Cmdlet("Update", "OCIDatabaseDbNode", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbNode), typeof(Oci.DatabaseService.Responses.UpdateDbNodeResponse) })]
    public class UpdateOCIDatabaseDbNode : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the specified database node.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the specified database node.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpdateDbNodeRequest
                {
                    DbNodeId = DbNodeId,
        ...
        private void HandleOutput(UpdateDbNodeRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateDbNode(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateDbNode(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbNode);
        }

        private UpdateDbNodeResponse response;
