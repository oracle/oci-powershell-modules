...
{
    [Cmdlet("Get", "OCITenantmanagercontrolplaneLinksList")]
    [OutputType(new System.Type[] { typeof(Oci.TenantmanagercontrolplaneService.Models.LinkCollection), typeof(Oci.TenantmanagercontrolplaneService.Responses.ListLinksResponse) })]
    public class GetOCITenantmanagercontrolplaneLinksList : OCILinkCmdlet
    {
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The ID of the parent tenancy this link is associated with.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListLinksResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.LinkCollection, true);
                }
        ...
        {
            IEnumerable<ListLinksResponse> DefaultRequest(ListLinksRequest request) => Enumerable.Repeat(client.ListLinks(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListLinksResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListLinksResponse response;
        private delegate IEnumerable<ListLinksResponse> RequestDelegate(ListLinksRequest request);
