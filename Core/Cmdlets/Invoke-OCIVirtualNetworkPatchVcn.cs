...
{
    [Cmdlet("Invoke", "OCIVirtualNetworkPatchVcn", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.PatchVcnResponse) })]
    public class InvokeOCIVirtualNetworkPatchVcn : OCIVirtualNetworkCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for patching a VCN.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for patching a VCN.", ParameterSetName = Default)]
        ...
        private void HandleOutput(PatchVcnRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForPatchVcn(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.PatchVcn(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private PatchVcnResponse response;
