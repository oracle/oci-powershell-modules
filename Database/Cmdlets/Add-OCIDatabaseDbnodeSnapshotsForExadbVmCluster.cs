...
{
    [Cmdlet("Add", "OCIDatabaseDbnodeSnapshotsForExadbVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbnodeSnapshots), typeof(Oci.DatabaseService.Responses.AddDbnodeSnapshotsForExadbVmClusterResponse) })]
    public class AddOCIDatabaseDbnodeSnapshotsForExadbVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to add a database node snapshots to the Exadb VM cluster..", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to add a database node snapshots to the Exadb VM cluster..", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new AddDbnodeSnapshotsForExadbVmClusterRequest
                {
        ...
        private void HandleOutput(AddDbnodeSnapshotsForExadbVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForAddDbnodeSnapshotsForExadbVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.AddDbnodeSnapshotsForExadbVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbnodeSnapshots);
        }

        private AddDbnodeSnapshotsForExadbVmClusterResponse response;
