...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudDbSystemDiscoveriesList")]
    ...
    public class GetOCIDatabasemanagementCloudDbSystemDiscoveriesList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudDbSystemDiscoveriesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudDbSystemDiscoveriesRequest
                {
                ...
                IEnumerable<ListCloudDbSystemDiscoveriesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudDbSystemDiscoveryCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudDbSystemDiscoveriesResponse> DefaultRequest(ListCloudDbSystemDiscoveriesRequest request) => Enumerable.Repeat(client.ListCloudDbSystemDiscoveries(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudDbSystemDiscoveriesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudDbSystemDiscoveriesResponse response;
        private delegate IEnumerable<ListCloudDbSystemDiscoveriesResponse> RequestDelegate(ListCloudDbSystemDiscoveriesRequest request);
