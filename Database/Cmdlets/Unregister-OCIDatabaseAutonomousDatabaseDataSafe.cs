...
{
    [Cmdlet("Unregister", "OCIDatabaseAutonomousDatabaseDataSafe", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.DeregisterAutonomousDatabaseDataSafeResponse) })]
    public class UnregisterOCIDatabaseAutonomousDatabaseDataSafe : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for deregistering an Autonomous AI Database with Data Safe.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for deregistering an Autonomous AI Database with Data Safe.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new DeregisterAutonomousDatabaseDataSafeRequest
                {
        ...
        private void HandleOutput(DeregisterAutonomousDatabaseDataSafeRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForDeregisterAutonomousDatabaseDataSafe(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.DeregisterAutonomousDatabaseDataSafe(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private DeregisterAutonomousDatabaseDataSafeResponse response;
