...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudAsmUsersList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudAsmUserCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudAsmUsersResponse) })]
    public class GetOCIDatabasemanagementCloudAsmUsersList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudAsmUsersRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudAsmUsersRequest
                {
                ...
                IEnumerable<ListCloudAsmUsersResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudAsmUserCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudAsmUsersResponse> DefaultRequest(ListCloudAsmUsersRequest request) => Enumerable.Repeat(client.ListCloudAsmUsers(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudAsmUsersResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudAsmUsersResponse response;
        private delegate IEnumerable<ListCloudAsmUsersResponse> RequestDelegate(ListCloudAsmUsersRequest request);
