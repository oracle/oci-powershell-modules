...
{
    [Cmdlet("Import", "OCIDatabaseTransportableTablespace", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AutonomousDatabase), typeof(Oci.DatabaseService.Responses.ImportTransportableTablespaceResponse) })]
    public class ImportOCIDatabaseTransportableTablespace : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for importing transportable tablespace for the Autonomous AI Database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for importing transportable tablespace for the Autonomous AI Database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ImportTransportableTablespaceRequest
                {
        ...
        private void HandleOutput(ImportTransportableTablespaceRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForImportTransportableTablespace(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ImportTransportableTablespace(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AutonomousDatabase);
        }

        private ImportTransportableTablespaceResponse response;
