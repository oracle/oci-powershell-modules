...
{
    [Cmdlet("New", "OCIDatabaseScheduledAction", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ScheduledAction), typeof(Oci.DatabaseService.Responses.CreateScheduledActionResponse) })]
    public class NewOCIDatabaseScheduledAction : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create Scheduled Action.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create Scheduled Action.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateScheduledActionRequest
                {
        ...
        private void HandleOutput(CreateScheduledActionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateScheduledAction(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateScheduledAction(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ScheduledAction);
        }

        private CreateScheduledActionResponse response;
