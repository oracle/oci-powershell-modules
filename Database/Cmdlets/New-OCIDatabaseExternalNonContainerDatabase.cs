...
{
    [Cmdlet("New", "OCIDatabaseExternalNonContainerDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExternalNonContainerDatabase), typeof(Oci.DatabaseService.Responses.CreateExternalNonContainerDatabaseResponse) })]
    public class NewOCIDatabaseExternalNonContainerDatabase : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new external non-container database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new external non-container database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateExternalNonContainerDatabaseRequest
                {
        ...
        private void HandleOutput(CreateExternalNonContainerDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExternalNonContainerDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExternalNonContainerDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExternalNonContainerDatabase);
        }

        private CreateExternalNonContainerDatabaseResponse response;
