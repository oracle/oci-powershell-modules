...
{
    [Cmdlet("Move", "OCIDatabaseCloudAutonomousVmClusterSubscription", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeCloudAutonomousVmClusterSubscriptionResponse) })]
    public class MoveOCIDatabaseCloudAutonomousVmClusterSubscription : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a Cloud Autonomous VM cluster with a different subscription.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a Cloud Autonomous VM cluster with a different subscription.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeCloudAutonomousVmClusterSubscriptionRequest
                {
        ...
        private void HandleOutput(ChangeCloudAutonomousVmClusterSubscriptionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeCloudAutonomousVmClusterSubscription(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeCloudAutonomousVmClusterSubscription(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeCloudAutonomousVmClusterSubscriptionResponse response;
