...
{
    [Cmdlet("New", "OCIDatabaseOneoffPatch", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.OneoffPatch), typeof(Oci.DatabaseService.Responses.CreateOneoffPatchResponse) })]
    public class NewOCIDatabaseOneoffPatch : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a one-off patch to download.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a one-off patch to download.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateOneoffPatchRequest
                {
        ...
        private void HandleOutput(CreateOneoffPatchRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateOneoffPatch(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateOneoffPatch(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.OneoffPatch);
        }

        private CreateOneoffPatchResponse response;
