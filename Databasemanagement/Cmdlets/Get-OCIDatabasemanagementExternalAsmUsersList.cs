...
{
    [Cmdlet("Get", "OCIDatabasemanagementExternalAsmUsersList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.ExternalAsmUserCollection), typeof(Oci.DatabasemanagementService.Responses.ListExternalAsmUsersResponse) })]
    public class GetOCIDatabasemanagementExternalAsmUsersList : OCIDbManagementCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm) of the external ASM.")]
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListExternalAsmUsersRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListExternalAsmUsersRequest
                {
                ...
                IEnumerable<ListExternalAsmUsersResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ExternalAsmUserCollection, true);
                }
        ...
        {
            IEnumerable<ListExternalAsmUsersResponse> DefaultRequest(ListExternalAsmUsersRequest request) => Enumerable.Repeat(client.ListExternalAsmUsers(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListExternalAsmUsersResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListExternalAsmUsersResponse response;
        private delegate IEnumerable<ListExternalAsmUsersResponse> RequestDelegate(ListExternalAsmUsersRequest request);
