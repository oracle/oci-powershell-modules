...
{
    [Cmdlet("New", "OCIDatabaseExternalPluggableDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExternalPluggableDatabase), typeof(Oci.DatabaseService.Responses.CreateExternalPluggableDatabaseResponse) })]
    public class NewOCIDatabaseExternalPluggableDatabase : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new external pluggable database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new external pluggable database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateExternalPluggableDatabaseRequest
                {
        ...
        private void HandleOutput(CreateExternalPluggableDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExternalPluggableDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExternalPluggableDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExternalPluggableDatabase);
        }

        private CreateExternalPluggableDatabaseResponse response;
