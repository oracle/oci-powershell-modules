...
{
    [Cmdlet("Invoke", "OCIDatabaseExecuteDbSystemOsPatch", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ExecuteDbSystemOsPatchResponse) })]
    public class InvokeOCIDatabaseExecuteDbSystemOsPatch : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to execute an OS patch action on a DB System.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to execute an OS patch action on a DB System.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ExecuteDbSystemOsPatchRequest
                {
        ...
        private void HandleOutput(ExecuteDbSystemOsPatchRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForExecuteDbSystemOsPatch(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ExecuteDbSystemOsPatch(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ExecuteDbSystemOsPatchResponse response;
