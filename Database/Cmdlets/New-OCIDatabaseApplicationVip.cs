...
{
    [Cmdlet("New", "OCIDatabaseApplicationVip", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ApplicationVip), typeof(Oci.DatabaseService.Responses.CreateApplicationVipResponse) })]
    public class NewOCIDatabaseApplicationVip : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new application virtual IP (VIP) address.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a new application virtual IP (VIP) address.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateApplicationVipRequest
                {
        ...
        private void HandleOutput(CreateApplicationVipRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateApplicationVip(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateApplicationVip(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ApplicationVip);
        }

        private CreateApplicationVipResponse response;
