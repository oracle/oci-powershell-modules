...
{
    [Cmdlet("Get", "OCIFleetappsmanagementProvisionsList")]
    [OutputType(new System.Type[] { typeof(Oci.FleetappsmanagementService.Models.ProvisionCollection), typeof(Oci.FleetappsmanagementService.Responses.ListProvisionsResponse) })]
    public class GetOCIFleetappsmanagementProvisionsList : OCIFleetAppsManagementProvisionCmdlet
    {
        ...
        public System.Nullable<Oci.FleetappsmanagementService.Models.Provision.LifecycleStateEnum> LifecycleState { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to return only resources that match the entire display name given.")]
        public string DisplayName { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Unique identifier or OCID for listing a single provision by id. Either compartmentId or id must be provided.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListProvisionsRequest
                {
                ...
                IEnumerable<ListProvisionsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ProvisionCollection, true);
                }
        ...
        {
            IEnumerable<ListProvisionsResponse> DefaultRequest(ListProvisionsRequest request) => Enumerable.Repeat(client.ListProvisions(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListProvisionsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListProvisionsResponse response;
        private delegate IEnumerable<ListProvisionsResponse> RequestDelegate(ListProvisionsRequest request);
