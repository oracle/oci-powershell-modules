...
{
    [Cmdlet("Enable", "OCIDatabaseExternalNonContainerDatabaseDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.EnableExternalNonContainerDatabaseDatabaseManagementResponse) })]
    public class EnableOCIDatabaseExternalNonContainerDatabaseDatabaseManagement : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The external non-container database [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The external non-container database [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        public string ExternalNonContainerDatabaseId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management Service for an external non-container database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management Service for an external non-container database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableExternalNonContainerDatabaseDatabaseManagementRequest
                {
        ...
        private void HandleOutput(EnableExternalNonContainerDatabaseDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableExternalNonContainerDatabaseDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableExternalNonContainerDatabaseDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private EnableExternalNonContainerDatabaseDatabaseManagementResponse response;
