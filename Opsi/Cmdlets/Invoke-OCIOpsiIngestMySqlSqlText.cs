...
{
    [Cmdlet("Invoke", "OCIOpsiIngestMySqlSqlText")]
    [OutputType(new System.Type[] { typeof(Oci.OpsiService.Models.IngestMySqlSqlTextResponseDetails), typeof(Oci.OpsiService.Responses.IngestMySqlSqlTextResponse) })]
    public class InvokeOCIOpsiIngestMySqlSqlText : OCIOperationsInsightsCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Collection of SQL text objects for a particular database.")]
...
*Note:* Retry tokens can be invalidated before the 24 hour time limit due to conflicting operations, such as a resource being deleted or purged from the system.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new IngestMySqlSqlTextRequest
                {
                ...
                response = client.IngestMySqlSqlText(request).GetAwaiter().GetResult();
                WriteOutput(response, response.IngestMySqlSqlTextResponseDetails);
        ...
        private IngestMySqlSqlTextResponse response;
    }
}
