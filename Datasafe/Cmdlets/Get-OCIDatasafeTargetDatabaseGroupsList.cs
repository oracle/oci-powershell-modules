...
{
    [Cmdlet("Get", "OCIDatasafeTargetDatabaseGroupsList")]
    [OutputType(new System.Type[] { typeof(Oci.DatasafeService.Models.TargetDatabaseGroupCollection), typeof(Oci.DatasafeService.Responses.ListTargetDatabaseGroupsResponse) })]
    public class GetOCIDatasafeTargetDatabaseGroupsList : OCIDataSafeCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The scim query filter parameter accepts filter expressions that use the syntax described in Section 3.2.2.2 of the System for Cross-Domain Identity Management (SCIM) specification, which is available at [RFC3339](https://tools.ietf.org/html/draft-ietf-scim-api-12). In SCIM filtering expressions, text, date, and time values must be enclosed in quotation marks, with date and time values using ISO-8601 format. (Numeric and boolean values should not be quoted.) Ex:** filter=(targetDatabaseId eq 'ocid1.datasafetargetdatabase.oc1.iad.abuwcljr3u2va4ba5wek53idpe5qq5kkbigzclscc6mysfecxzjt5dgmxqza')")]
        public string Filter { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The sorting field for your request. You can only specify a single sorting order (sortOrder). The default order for timeCreated is descending.")]
        public System.Nullable<Oci.DatasafeService.Requests.ListTargetDatabaseGroupsRequest.SortByEnum> SortBy { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to retrieve resources that exclusively align with the designated lifecycle state.")]
        public System.Nullable<Oci.DatasafeService.Models.TargetDatabaseGroupLifecycleState> LifecycleState { get; set; }
        ...
        public System.Nullable<Oci.DatasafeService.Requests.ListTargetDatabaseGroupsRequest.AccessLevelEnum> AccessLevel { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The sort order to use, either ascending (ASC) or descending (DESC).")]
        public System.Nullable<Oci.DatasafeService.Requests.ListTargetDatabaseGroupsRequest.SortOrderEnum> SortOrder { get; set; }
...
**Example:** 2016-12-19T16:39:57.600Z")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListTargetDatabaseGroupsRequest
                {
                ...
                IEnumerable<ListTargetDatabaseGroupsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.TargetDatabaseGroupCollection, true);
                }
        ...
        {
            IEnumerable<ListTargetDatabaseGroupsResponse> DefaultRequest(ListTargetDatabaseGroupsRequest request) => Enumerable.Repeat(client.ListTargetDatabaseGroups(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListTargetDatabaseGroupsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListTargetDatabaseGroupsResponse response;
        private delegate IEnumerable<ListTargetDatabaseGroupsResponse> RequestDelegate(ListTargetDatabaseGroupsRequest request);
