...
{
    [Cmdlet("Remove", "OCIDatabaseDbHome", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.High, DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.DeleteDbHomeResponse) })]
    public class RemoveOCIDatabaseDbHome : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The Database Home [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
...
This parameter is used in multiple APIs. Refer to the API description for details on how the operation uses it.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (!ConfirmDelete("OCIDatabaseDbHome", "Remove"))
            {
               return;
            }
            ...
            try
            {
                request = new DeleteDbHomeRequest
                {
                    DbHomeId = DbHomeId,
                    IfMatch = IfMatch,
        ...
        private void HandleOutput(DeleteDbHomeRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForDeleteDbHome(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.DeleteDbHome(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private DeleteDbHomeResponse response;
