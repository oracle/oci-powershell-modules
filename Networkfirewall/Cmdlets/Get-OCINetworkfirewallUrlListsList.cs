...
{
    [Cmdlet("Get", "OCINetworkfirewallUrlListsList")]
    [OutputType(new System.Type[] { typeof(Oci.NetworkfirewallService.Models.UrlListSummaryCollection), typeof(Oci.NetworkfirewallService.Responses.ListUrlListsResponse) })]
    public class GetOCINetworkfirewallUrlListsList : OCINetworkFirewallCmdlet
    {
        ...
        public System.Nullable<Oci.NetworkfirewallService.Requests.ListUrlListsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListUrlListsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.UrlListSummaryCollection, true);
                }
        ...
        {
            IEnumerable<ListUrlListsResponse> DefaultRequest(ListUrlListsRequest request) => Enumerable.Repeat(client.ListUrlLists(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListUrlListsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListUrlListsResponse response;
        private delegate IEnumerable<ListUrlListsResponse> RequestDelegate(ListUrlListsRequest request);
