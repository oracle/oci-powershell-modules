...
{
    [Cmdlet("Get", "OCIOspgatewayInvoiceLinesList")]
    [OutputType(new System.Type[] { typeof(Oci.OspgatewayService.Models.InvoiceLineCollection), typeof(Oci.OspgatewayService.Responses.ListInvoiceLinesResponse) })]
    public class GetOCIOspgatewayInvoiceLinesList : OCIInvoiceServiceCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListInvoiceLinesRequest
                {
                ...
                IEnumerable<ListInvoiceLinesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.InvoiceLineCollection, true);
                }
        ...
        {
            IEnumerable<ListInvoiceLinesResponse> DefaultRequest(ListInvoiceLinesRequest request) => Enumerable.Repeat(client.ListInvoiceLines(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListInvoiceLinesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListInvoiceLinesResponse response;
        private delegate IEnumerable<ListInvoiceLinesResponse> RequestDelegate(ListInvoiceLinesRequest request);
