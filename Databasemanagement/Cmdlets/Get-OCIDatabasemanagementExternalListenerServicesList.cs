...
{
    [Cmdlet("Get", "OCIDatabasemanagementExternalListenerServicesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.ExternalListenerServiceCollection), typeof(Oci.DatabasemanagementService.Responses.ListExternalListenerServicesResponse) })]
    public class GetOCIDatabasemanagementExternalListenerServicesList : OCIDbManagementCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the external listener.")]
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListExternalListenerServicesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListExternalListenerServicesRequest
                {
                ...
                IEnumerable<ListExternalListenerServicesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ExternalListenerServiceCollection, true);
                }
        ...
        {
            IEnumerable<ListExternalListenerServicesResponse> DefaultRequest(ListExternalListenerServicesRequest request) => Enumerable.Repeat(client.ListExternalListenerServices(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListExternalListenerServicesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListExternalListenerServicesResponse response;
        private delegate IEnumerable<ListExternalListenerServicesResponse> RequestDelegate(ListExternalListenerServicesRequest request);
