...
{
    [Cmdlet("Get", "OCIMulticloudSubscriptionsList")]
    [OutputType(new System.Type[] { typeof(Oci.MulticloudService.Models.MulticloudSubscriptionCollection), typeof(Oci.MulticloudService.Responses.ListMulticloudSubscriptionsResponse) })]
    public class GetOCIMulticloudSubscriptionsList : OCIMulticloudsubscriptionsCmdlet
    {
        ...
        public System.Nullable<Oci.MulticloudService.Requests.ListMulticloudSubscriptionsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListMulticloudSubscriptionsRequest
                {
                ...
                IEnumerable<ListMulticloudSubscriptionsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.MulticloudSubscriptionCollection, true);
                }
        ...
        {
            IEnumerable<ListMulticloudSubscriptionsResponse> DefaultRequest(ListMulticloudSubscriptionsRequest request) => Enumerable.Repeat(client.ListMulticloudSubscriptions(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListMulticloudSubscriptionsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListMulticloudSubscriptionsResponse response;
        private delegate IEnumerable<ListMulticloudSubscriptionsResponse> RequestDelegate(ListMulticloudSubscriptionsRequest request);
