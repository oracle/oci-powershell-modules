...
{
    [Cmdlet("Get", "OCINetworkfirewallServiceListsList")]
    [OutputType(new System.Type[] { typeof(Oci.NetworkfirewallService.Models.ServiceListSummaryCollection), typeof(Oci.NetworkfirewallService.Responses.ListServiceListsResponse) })]
    public class GetOCINetworkfirewallServiceListsList : OCINetworkFirewallCmdlet
    {
        ...
        public System.Nullable<Oci.NetworkfirewallService.Requests.ListServiceListsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListServiceListsRequest
                {
                ...
                IEnumerable<ListServiceListsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ServiceListSummaryCollection, true);
                }
        ...
        {
            IEnumerable<ListServiceListsResponse> DefaultRequest(ListServiceListsRequest request) => Enumerable.Repeat(client.ListServiceLists(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListServiceListsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListServiceListsResponse response;
        private delegate IEnumerable<ListServiceListsResponse> RequestDelegate(ListServiceListsRequest request);
