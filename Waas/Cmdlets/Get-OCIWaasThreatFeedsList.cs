...
{
    [Cmdlet("Get", "OCIWaasThreatFeedsList")]
    [OutputType(new System.Type[] { typeof(Oci.WaasService.Models.ThreatFeed), typeof(Oci.WaasService.Responses.ListThreatFeedsResponse) })]
    public class GetOCIWaasThreatFeedsList : OCIWaasCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListThreatFeedsRequest
                {
                ...
                IEnumerable<ListThreatFeedsResponse> responses = GetRequestDelegate().Invoke(request);
        ...
        {
            IEnumerable<ListThreatFeedsResponse> DefaultRequest(ListThreatFeedsRequest request) => Enumerable.Repeat(client.ListThreatFeeds(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListThreatFeedsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListThreatFeedsResponse response;
        private delegate IEnumerable<ListThreatFeedsResponse> RequestDelegate(ListThreatFeedsRequest request);
