...
{
    [Cmdlet("Invoke", "OCIDatabaseSwitchOverDataGuard", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.SwitchOverDataGuardResponse) })]
    public class InvokeOCIDatabaseSwitchOverDataGuard : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"A request to update an existing Data Guard member.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"A request to update an existing Data Guard member.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new SwitchOverDataGuardRequest
                {
        ...
        private void HandleOutput(SwitchOverDataGuardRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForSwitchOverDataGuard(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.SwitchOverDataGuard(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private SwitchOverDataGuardResponse response;
