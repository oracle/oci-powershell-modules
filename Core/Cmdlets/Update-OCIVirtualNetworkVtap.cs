...
{
    [Cmdlet("Update", "OCIVirtualNetworkVtap", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.Vtap), typeof(Oci.CoreService.Responses.UpdateVtapResponse) })]
    public class UpdateOCIVirtualNetworkVtap : OCIVirtualNetworkCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for updating a VTAP.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for updating a VTAP.", ParameterSetName = Default)]
        ...
        private void HandleOutput(UpdateVtapRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateVtap(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateVtap(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Vtap);
        }

        private UpdateVtapResponse response;
