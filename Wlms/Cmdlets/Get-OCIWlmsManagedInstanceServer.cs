/*
 * NOTE: Generated using OracleSDKGenerator, API Version: 20241101
 * DO NOT EDIT this file manually.
 *
 * Copyright (c) 2020, 2026, Oracle and/or its affiliates.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

using System;
using System.Management.Automation;
using Oci.WlmsService.Requests;
using Oci.WlmsService.Responses;
using Oci.WlmsService.Models;
using Oci.Common.Model;

namespace Oci.WlmsService.Cmdlets
{
    [Cmdlet("Get", "OCIWlmsManagedInstanceServer")]
    [OutputType(new System.Type[] { typeof(Oci.WlmsService.Models.Server), typeof(Oci.WlmsService.Responses.GetManagedInstanceServerResponse) })]
    public class GetOCIWlmsManagedInstanceServer : OCIWeblogicManagementServiceCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the instance.")]
        public string ManagedInstanceId { get; set; }

        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The unique identifier of a server.

**Note:** Not an [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm).")]
        public string ServerId { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The client request ID for tracing.")]
        public string OpcRequestId { get; set; }

        protected override void ProcessRecord()
        {
            base.ProcessRecord();
            GetManagedInstanceServerRequest request;

            try
            {
                request = new GetManagedInstanceServerRequest
                {
                    ManagedInstanceId = ManagedInstanceId,
                    ServerId = ServerId,
                    OpcRequestId = OpcRequestId
                };

                response = client.GetManagedInstanceServer(request).GetAwaiter().GetResult();
                WriteOutput(response, response.Server);
                FinishProcessing(response);
            }
            catch (OciException ex)
            {
                TerminatingErrorDuringExecution(ex);
            }
            catch (Exception ex)
            {
                TerminatingErrorDuringExecution(ex);
            }
        }

        protected override void StopProcessing()
        {
            base.StopProcessing();
            TerminatingErrorDuringExecution(new OperationCanceledException("Cmdlet execution interrupted"));
        }

        private GetManagedInstanceServerResponse response;
    }
}
