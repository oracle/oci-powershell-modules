...
{
    [Cmdlet("Update", "OCIDatabaseDbSystem", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbSystem), typeof(Oci.DatabaseService.Responses.UpdateDbSystemResponse) })]
    public class UpdateOCIDatabaseDbSystem : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the properties of a DB system.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the properties of a DB system.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new UpdateDbSystemRequest
                {
        ...
        private void HandleOutput(UpdateDbSystemRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateDbSystem(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateDbSystem(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbSystem);
        }

        private UpdateDbSystemResponse response;
