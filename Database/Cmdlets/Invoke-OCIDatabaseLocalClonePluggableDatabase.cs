...
{
    [Cmdlet("Invoke", "OCIDatabaseLocalClonePluggableDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabase), typeof(Oci.DatabaseService.Responses.LocalClonePluggableDatabaseResponse) })]
    public class InvokeOCIDatabaseLocalClonePluggableDatabase : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to clone a pluggable database locally.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to clone a pluggable database locally.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new LocalClonePluggableDatabaseRequest
                {
        ...
        private void HandleOutput(LocalClonePluggableDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForLocalClonePluggableDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.LocalClonePluggableDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabase);
        }

        private LocalClonePluggableDatabaseResponse response;
