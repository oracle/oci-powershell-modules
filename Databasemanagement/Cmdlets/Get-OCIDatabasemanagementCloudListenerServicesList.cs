...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudListenerServicesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudListenerServiceCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudListenerServicesResponse) })]
    public class GetOCIDatabasemanagementCloudListenerServicesList : OCIDbManagementCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the cloud listener.")]
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudListenerServicesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudListenerServicesRequest
                {
                ...
                IEnumerable<ListCloudListenerServicesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudListenerServiceCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudListenerServicesResponse> DefaultRequest(ListCloudListenerServicesRequest request) => Enumerable.Repeat(client.ListCloudListenerServices(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudListenerServicesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudListenerServicesResponse response;
        private delegate IEnumerable<ListCloudListenerServicesResponse> RequestDelegate(ListCloudListenerServicesRequest request);
