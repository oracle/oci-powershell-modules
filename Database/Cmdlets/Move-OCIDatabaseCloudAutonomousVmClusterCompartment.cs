...
{
    [Cmdlet("Move", "OCIDatabaseCloudAutonomousVmClusterCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeCloudAutonomousVmClusterCompartmentResponse) })]
    public class MoveOCIDatabaseCloudAutonomousVmClusterCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move cloud Autonomous VM cluster to a different compartment", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move cloud Autonomous VM cluster to a different compartment", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeCloudAutonomousVmClusterCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeCloudAutonomousVmClusterCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeCloudAutonomousVmClusterCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeCloudAutonomousVmClusterCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeCloudAutonomousVmClusterCompartmentResponse response;
