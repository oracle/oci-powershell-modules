...
{
    [Cmdlet("Get", "OCIOsmanagementhubSoftwarePackageSoftwareSourcesList")]
    [OutputType(new System.Type[] { typeof(Oci.OsmanagementhubService.Models.SoftwareSourceCollection), typeof(Oci.OsmanagementhubService.Responses.ListSoftwarePackageSoftwareSourcesResponse) })]
    public class GetOCIOsmanagementhubSoftwarePackageSoftwareSourcesList : OCISoftwareSourceCmdlet
    {
...
Example: `3`")]
        ...
        public System.Nullable<Oci.OsmanagementhubService.Requests.ListSoftwarePackageSoftwareSourcesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListSoftwarePackageSoftwareSourcesRequest
                {
                ...
                IEnumerable<ListSoftwarePackageSoftwareSourcesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.SoftwareSourceCollection, true);
                }
        ...
        {
            IEnumerable<ListSoftwarePackageSoftwareSourcesResponse> DefaultRequest(ListSoftwarePackageSoftwareSourcesRequest request) => Enumerable.Repeat(client.ListSoftwarePackageSoftwareSources(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListSoftwarePackageSoftwareSourcesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListSoftwarePackageSoftwareSourcesResponse response;
        private delegate IEnumerable<ListSoftwarePackageSoftwareSourcesResponse> RequestDelegate(ListSoftwarePackageSoftwareSourcesRequest request);
