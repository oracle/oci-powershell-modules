...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudAsmDiskGroupsList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudAsmDiskGroupCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudAsmDiskGroupsResponse) })]
    public class GetOCIDatabasemanagementCloudAsmDiskGroupsList : OCIDbManagementCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/Content/General/Concepts/identifiers.htm) of the cloud ASM.")]
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudAsmDiskGroupsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudAsmDiskGroupsRequest
                {
                ...
                IEnumerable<ListCloudAsmDiskGroupsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudAsmDiskGroupCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudAsmDiskGroupsResponse> DefaultRequest(ListCloudAsmDiskGroupsRequest request) => Enumerable.Repeat(client.ListCloudAsmDiskGroups(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudAsmDiskGroupsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudAsmDiskGroupsResponse response;
        private delegate IEnumerable<ListCloudAsmDiskGroupsResponse> RequestDelegate(ListCloudAsmDiskGroupsRequest request);
