...
{
    [Cmdlet("Enable", "OCIOpsiAutonomousDatabaseInsightAdvancedFeatures")]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.OpsiService.Responses.EnableAutonomousDatabaseInsightAdvancedFeaturesResponse) })]
    public class EnableOCIOpsiAutonomousDatabaseInsightAdvancedFeatures : OCIOperationsInsightsCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Connection Details for the Autonomous Database in Operations Insights.")]
...
*Note:* Retry tokens can be invalidated before the 24 hour time limit due to conflicting operations, such as a resource being deleted or purged from the system.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableAutonomousDatabaseInsightAdvancedFeaturesRequest
                {
                ...
                response = client.EnableAutonomousDatabaseInsightAdvancedFeatures(request).GetAwaiter().GetResult();
        ...
        private EnableAutonomousDatabaseInsightAdvancedFeaturesResponse response;
    }
}
