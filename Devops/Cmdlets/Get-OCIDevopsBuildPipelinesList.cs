...
{
    [Cmdlet("Get", "OCIDevopsBuildPipelinesList")]
    [OutputType(new System.Type[] { typeof(Oci.DevopsService.Models.BuildPipelineCollection), typeof(Oci.DevopsService.Responses.ListBuildPipelinesResponse) })]
    public class GetOCIDevopsBuildPipelinesList : OCIDevopsCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to return only build pipelines that matches the given lifecycle state.")]
        public System.Nullable<Oci.DevopsService.Models.BuildPipeline.LifecycleStateEnum> LifecycleState { get; set; }
        ...
        public System.Nullable<Oci.DevopsService.Requests.ListBuildPipelinesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListBuildPipelinesRequest
                {
                    Id = Id,
                    ProjectId = ProjectId,
                ...
                IEnumerable<ListBuildPipelinesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.BuildPipelineCollection, true);
                }
        ...
        {
            IEnumerable<ListBuildPipelinesResponse> DefaultRequest(ListBuildPipelinesRequest request) => Enumerable.Repeat(client.ListBuildPipelines(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListBuildPipelinesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListBuildPipelinesResponse response;
        private delegate IEnumerable<ListBuildPipelinesResponse> RequestDelegate(ListBuildPipelinesRequest request);
