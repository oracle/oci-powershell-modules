...
{
    [Cmdlet("Get", "OCIOdaSkill", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.OdaService.Models.Skill), typeof(Oci.OdaService.Responses.GetSkillResponse) })]
    public class GetOCIOdaSkill : OCIManagementCmdlet
    {
        ...
        private void HandleOutput(GetSkillRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForSkill(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetSkill(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Skill);
        }

        private GetSkillResponse response;
