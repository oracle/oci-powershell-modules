...
{
    [Cmdlet("Get", "OCIVirtualNetworkIpInventoryList", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.IpInventoryCollection), typeof(Oci.CoreService.Responses.ListIpInventoryResponse) })]
    public class GetOCIVirtualNetworkIpInventoryList : OCIVirtualNetworkCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details required to list the IP Inventory data.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details required to list the IP Inventory data.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListIpInventoryRequest
                {
        ...
        private void HandleOutput(ListIpInventoryRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForListIpInventory(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ListIpInventory(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.IpInventoryCollection);
        }

        private ListIpInventoryResponse response;
