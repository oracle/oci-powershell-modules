...
{
    [Cmdlet("Get", "OCIWorkRequestsWorkRequest", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.MysqlService.Models.WorkRequest), typeof(Oci.MysqlService.Responses.GetWorkRequestResponse) })]
    public class GetOCIWorkRequestsWorkRequest : OCIWorkRequestsCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"the ID of the WorkRequest", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"the ID of the WorkRequest", ParameterSetName = Default)]
        public string WorkRequestId { get; set; }

        
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"For conditional requests. In the GET call for a resource, set the `If-None-Match` header to the value of the ETag from a previous GET (or POST or PUT) response for that resource. The server will return with either a 304 Not Modified response if the resource has not changed, or a 200 OK response with the updated representation.", ParameterSetName = StatusParamSet)]
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Customer-defined unique identifier for the request. If you need to contact Oracle about a specific request, please provide the request ID that you supplied in this header with the request.", ParameterSetName = StatusParamSet)]
        ...
        public Oci.MysqlService.Models.WorkRequestOperationStatus[] WaitForStatus { get; set; }
