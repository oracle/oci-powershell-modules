...
{
    [Cmdlet("Move", "OCIDatabaseCloudDbSystemSubscription", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeCloudDbSystemSubscriptionResponse) })]
    public class MoveOCIDatabaseCloudDbSystemSubscription : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a cloud DB system with a different subscription.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a cloud DB system with a different subscription.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeCloudDbSystemSubscriptionRequest
                {
        ...
        private void HandleOutput(ChangeCloudDbSystemSubscriptionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeCloudDbSystemSubscription(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeCloudDbSystemSubscription(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeCloudDbSystemSubscriptionResponse response;
