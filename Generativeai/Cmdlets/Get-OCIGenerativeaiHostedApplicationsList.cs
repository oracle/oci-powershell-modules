...
{
    [Cmdlet("Get", "OCIGenerativeaiHostedApplicationsList")]
    [OutputType(new System.Type[] { typeof(Oci.GenerativeaiService.Models.HostedApplicationCollection), typeof(Oci.GenerativeaiService.Responses.ListHostedApplicationsResponse) })]
    public class GetOCIGenerativeaiHostedApplicationsList : OCIGenerativeAiCmdlet
    {
        ...
        public System.Nullable<Oci.GenerativeaiService.Requests.ListHostedApplicationsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListHostedApplicationsRequest
                {
                ...
                IEnumerable<ListHostedApplicationsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.HostedApplicationCollection, true);
                }
        ...
        {
            IEnumerable<ListHostedApplicationsResponse> DefaultRequest(ListHostedApplicationsRequest request) => Enumerable.Repeat(client.ListHostedApplications(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListHostedApplicationsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListHostedApplicationsResponse response;
        private delegate IEnumerable<ListHostedApplicationsResponse> RequestDelegate(ListHostedApplicationsRequest request);
