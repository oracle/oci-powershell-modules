...
{
    [Cmdlet("New", "OCIDatabaseAdvancedClusterFileSystem", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.AdvancedClusterFileSystem), typeof(Oci.DatabaseService.Responses.CreateAdvancedClusterFileSystemResponse) })]
    public class NewOCIDatabaseAdvancedClusterFileSystem : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to an advanced cluster file system.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to an advanced cluster file system.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateAdvancedClusterFileSystemRequest
                {
        ...
        private void HandleOutput(CreateAdvancedClusterFileSystemRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateAdvancedClusterFileSystem(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateAdvancedClusterFileSystem(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.AdvancedClusterFileSystem);
        }

        private CreateAdvancedClusterFileSystemResponse response;
