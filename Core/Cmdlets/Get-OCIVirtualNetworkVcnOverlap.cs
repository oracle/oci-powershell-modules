...
{
    [Cmdlet("Get", "OCIVirtualNetworkVcnOverlap", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.IpInventoryVcnOverlapCollection), typeof(Oci.CoreService.Responses.GetVcnOverlapResponse) })]
    public class GetOCIVirtualNetworkVcnOverlap : OCIVirtualNetworkCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Specify the [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the VCN.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Specify the [OCID](https://docs.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the VCN.", ParameterSetName = Default)]
        public string VcnId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Lists details of the IP Inventory VCN overlap data.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Lists details of the IP Inventory VCN overlap data.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new GetVcnOverlapRequest
                {
                    VcnId = VcnId,
        ...
        private void HandleOutput(GetVcnOverlapRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForGetVcnOverlap(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.GetVcnOverlap(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.IpInventoryVcnOverlapCollection);
        }

        private GetVcnOverlapResponse response;
