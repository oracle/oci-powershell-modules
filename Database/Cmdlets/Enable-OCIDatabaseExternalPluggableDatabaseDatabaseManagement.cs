...
{
    [Cmdlet("Enable", "OCIDatabaseExternalPluggableDatabaseDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.EnableExternalPluggableDatabaseDatabaseManagementResponse) })]
    public class EnableOCIDatabaseExternalPluggableDatabaseDatabaseManagement : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The ExternalPluggableDatabaseId [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The ExternalPluggableDatabaseId [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = Default)]
        public string ExternalPluggableDatabaseId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management Service for an external database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management Service for an external database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableExternalPluggableDatabaseDatabaseManagementRequest
                {
        ...
        private void HandleOutput(EnableExternalPluggableDatabaseDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableExternalPluggableDatabaseDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableExternalPluggableDatabaseDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private EnableExternalPluggableDatabaseDatabaseManagementResponse response;
