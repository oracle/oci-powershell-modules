...
{
    [Cmdlet("Enable", "OCIDatabaseExternalContainerDatabaseStackMonitoring", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.EnableExternalContainerDatabaseStackMonitoringResponse) })]
    public class EnableOCIDatabaseExternalContainerDatabaseStackMonitoring : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to enable Stack Monitoring on the external container database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to enable Stack Monitoring on the external container database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableExternalContainerDatabaseStackMonitoringRequest
                {
        ...
        private void HandleOutput(EnableExternalContainerDatabaseStackMonitoringRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableExternalContainerDatabaseStackMonitoring(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableExternalContainerDatabaseStackMonitoring(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private EnableExternalContainerDatabaseStackMonitoringResponse response;
