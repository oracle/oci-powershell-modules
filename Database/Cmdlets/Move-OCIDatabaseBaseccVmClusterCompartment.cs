...
{
    [Cmdlet("Move", "OCIDatabaseBaseccVmClusterCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeBaseccVmClusterCompartmentResponse) })]
    public class MoveOCIDatabaseBaseccVmClusterCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move the BaseDB-C@C VM cluster to a different compartment.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move the BaseDB-C@C VM cluster to a different compartment.", ParameterSetName = Default)]
        public ChangeBaseccVmClusterCompartmentDetails ChangeBaseccVmClusterCompartmentDetails { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The BaseDB-C@C VM cluster [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm).", ParameterSetName = StatusParamSet)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeBaseccVmClusterCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeBaseccVmClusterCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeBaseccVmClusterCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeBaseccVmClusterCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeBaseccVmClusterCompartmentResponse response;
