...
### Breaking Changes 
- The field `BillingDetails` was changed from `BillingDetails` to `List<BillingDetails>` in the model `SubscriptionDetails` in the Service Enablement Lifecycle Framework service in the .NET SDK 
- The nested enum `PricingPlan.BillingFrequencyEnum` was removed and the type of field `BillingFrequency` was changed to `BillingFrequency` in the model `PricingPlan` in the Service Enablement Lifecycle Framework service in the .NET SDK 
- The fields `PricingPlanKey` and `BillingModel` were added as required fields to the model `BillingDetails` in the Service Enablement Lifecycle Framework service in the .NET SDK 
- The enum value `SelfTokenId` was removed from the `SortBy` enum in the requests `ListSubscriptionsRequest` and `ListingSubscriptionsRequest` in the Service Enablement Lifecycle Framework service in the .NET SDK 
- The endpoints for `ActivateSubscription`, `ListingSubscriptions`, and `ResolveSubscription` now use `/partners` instead of `/partner` in the Service Enablement Lifecycle Framework service in the .NET SDK 
- The enums `TypeEnum`, `LifecycleStateEnum`, and `InfrastructureTypeEnum` in the model `AutonomousContainerDatabaseBackup` now deserialize unknown values as `UnknownEnumValue` instead of throwing an exception, and the underlying numeric values of their existing enum members have changed, in the Database service in the .NET SDK
...
### Breaking Changes 
- The fields `Id`, `ResolverId`, `FreeformTags`, and `DefinedTags` were added as required fields to the models `ResolverEndpoint` and `ResolverEndpointSummary` in the DNS service in the .NET SDK 
- The field `SecurityAttributes` was added as a required field to the model `ResolverVnicEndpointSummary` in the DNS service in the .NET SDK
...
### Breaking Changes 
- The model `CustomerLoggingDetails` was removed from the OpenSearch service in the .NET SDK 
- The model `ReviveWorkflowInstanceDetails` was removed from the Organizations service in the .NET SDK
...
- The field `CustomerSupportKey` was removed from the model `TenancyInformation` in the Customer Incident Management service in the .NET SDK 
- The field `Csi` was removed from the models `CreateIncident`, `CreateUserDetails`, and `User` in the Customer Incident Management service in the .NET SDK 
- The field `Csi` was removed from the requests `GetIncidentRequest`, `ListIncidentResourceTypesRequest`, `ListIncidentsRequest`, `PutAttachmentRequest`, `UpdateIncidentRequest`, and `ValidateUserRequest` in the Customer Incident Management service in the .NET SDK
...
### Breaking Changes 
- The fields AvailabilityDomain and SubnetId were made mandatory in the model PlacementConfiguration in the Kubernetes Engine service in the .NET SDK 
- The models CreateIdcsAuthConfigurationDetails, CreateIdcsCustomServiceAuthConfigurationDetails, IdcsAuthConfiguration, and IdcsCustomServiceAuthConfiguration were removed in the Data Science service in the .NET SDK 
- The enum values Idcs and IdcsCustomService were removed from the model AuthConfiguration.TypeEnum in the Data Science service in the .NET SDK
...
### Breaking Changes 
- The fields `ApplianceImageSummary` and `DefinedTags` removed from models `ApplianceImageSummary` and `SupportedCloudRegionSummary` in the Cloud Bridge service in the .NET SDK
...
### Breaking Changes 
- The field `OpcCompartmentId` was removed from the model `GenerateSqlFromNlRequest` in the Generative AI Data service in the .NET SDK 
- The field `CompartmentId` was removed from the model `GenerateSqlFromNlJob` in the Generative AI Data service in the .NET SDK 
...
### Breaking Changes 
- The request `UpdateComputeHostRequest` has been renamed to `UpdateComputeHostsRequest`in the Compute service in the .NET SDK 
- The request `GetComputeHostRequest` has been renamed to `GetComputeHostsRequest`in the Compute service in the .NET SDK 
- The model `UpdateComputeHostDetails` has been renamed to `UpdateComputeHostsDetails`in the Compute service in the .NET SDK 
- The response `GetComputeHostResponse` has been renamed to `GetComputeHostsResponse`in the Compute service in the .NET SDK 
- The response `UpdateComputeHostResponse` has been renamed to `UpdateComputeHostsResponse`in the Compute service in the .NET SDK
...
### Breaking Changes 
- The field `ContentLocation` was changed to `Location` in the model `InvokeRawCommandResponse` in the OCI Internet of Things (OCI IoT) service in the .NET SDK 
- The field `LifecycleState` was changed to `NetworkAnchorLifecycleState` and its type was changed to `NetworkAnchorNetworkAnchorLifecycleStateEnum` in the models `NetworkAnchor` and `NetworkAnchorSummary` in the MultiCloud service in the .NET SDK 
- The type of field `NetworkAnchorConnectionStatus` has been changed to `NetworkAnchorConnectionStatusEnum` in the model `OciNetworkMetadata` in the MultiCloud service in the .NET SDK 
- The type of field `NetworkAnchorConnectionStatus` has been changed to `NetworkAnchorConnectionStatusEnum` in the model `OciNetworkMetadata` in the MultiCloud service in the .NET SDK
...
### Breaking Changes 
- The models `AddPublicIpPoolCapacityDetails`, `AddSubnetIpv6CidrDetails`, `AddVcnCidrDetails`, `AddVcnIpv6CidrDetails`, `AllowedIkeIpSecParameters`, `AllowedPhaseOneParameters`, `AllowedPhaseTwoParameters`, `BgpSessionInfo`, `ByoipAllocatedRangeCollection`, `ByoipAllocatedRangeSummary`, `ByoipRange`, `ByoipRangeCollection`, `ByoipRangeSummary`, `ByoipRangeVcnIpv6AllocationSummary`, `Byoipv6CidrDetails`, `CaptureFilter`, `ChangeByoipRangeCompartmentDetails`, `ChangeCaptureFilterCompartmentDetails`, `ChangeCpeCompartmentDetails`, `ChangeCrossConnectCompartmentDetails`, `ChangeCrossConnectGroupCompartmentDetails`, `ChangeDhcpOptionsCompartmentDetails`, `ChangeDrgCompartmentDetails`, `ChangeInternetGatewayCompartmentDetails`, `ChangeIpSecConnectionCompartmentDetails`, `ChangeLocalPeeringGatewayCompartmentDetails`, `ChangeNatGatewayCompartmentDetails`, `ChangeNetworkSecurityGroupCompartmentDetails`, `ChangePublicIpCompartmentDetails`, `ChangePublicIpPoolCompartmentDetails`, `ChangeRemotePeeringConnectionCompartmentDetails`, `ChangeRouteTableCompartmentDetails`, `ChangeSecurityListCompartmentDetails`, `ChangeServiceGatewayCompartmentDetails`, `ChangeSubnetCompartmentDetails`, `ChangeVcnCompartmentDetails`, `ChangeVirtualCircuitCompartmentDetails`, `ChangeVlanCompartmentDetails`, `ChangeVtapCompartmentDetails`, `CreateByoipRangeDetails`, `CreateCaptureFilterDetails`, `CreateIpSecConnectionTunnelDetails`, `CreateIpSecTunnelBgpSessionDetails`, `CreateIpSecTunnelEncryptionDomainDetails`, `CreateMacsecKey`, `CreateMacsecProperties`, `CreateNatGatewayDetails`, `CreatePublicIpPoolDetails`, `CreateVlanDetails`, `CreateVnicDetails`, `CreateVtapDetails`, `CpeDeviceConfigAnswer`, `CpeDeviceConfigQuestion`, `CpeDeviceInfo`, `CpeDeviceShapeDetail`, `CpeDeviceShapeSummary`, `CrossConnectMappingDetails`, `CrossConnectMappingDetailsCollection`, `DefaultPhaseOneParameters`, `DefaultPhaseTwoParameters`, `DpdConfig`, `DrgAttachmentMatchAllDrgRouteDistributionMatchCriteria`, `DrgRedundancyStatus`, `EncryptionDomainConfig`, `FastConnectProviderServiceKey`, `FlowLogCaptureFilterRuleDetails`, `IpSecConnectionTunnel`, `IpSecConnectionTunnelErrorDetails`, `IpSecConnectionTunnelSharedSecret`, `MacsecKey`, `MacsecProperties`, `ModifyVcnCidrDetails`, `NatGateway`, `PhaseOneConfigDetails`, `PhaseTwoConfigDetails`, `PublicIpPool`, `PublicIpPoolCollection`, `PublicIpPoolSummary`, `RemovePublicIpPoolCapacityDetails`, `RemoveSubnetIpv6CidrDetails`, `RemoveVcnCidrDetails`, `RemoveVcnIpv6CidrDetails`, `TunnelCpeDeviceConfig`, `TunnelPhaseOneDetails`, `TunnelPhaseTwoDetails`, `TunnelRouteSummary`, `TunnelSecurityAssociationSummary`, `UpdateByoipRangeDetails`, `UpdateCaptureFilterDetails`, `UpdateIpSecConnectionTunnelDetails`, `UpdateIpSecConnectionTunnelEncryptionDomainDetails`, `UpdateIpSecConnectionTunnelSharedSecretDetails`, `UpdateIpSecTunnelBgpSessionDetails`, `UpdateMacsecKey`, `UpdateMacsecProperties`, `UpdateNatGatewayDetails`, `UpdatePublicIpPoolDetails`, `UpdateTunnelCpeDeviceConfigDetails`, `UpdateVlanDetails`, `UpdateVtapDetails`, `VcnDnsResolverAssociation`, `Vlan`, `Vtap`, and `VtapCaptureFilterRuleDetails` have been removed in the Network Monitoring service in the .NET SDK 
- The field `CpeDeviceShapeId` was removed from the models `Cpe` and `UpdateCpeDetails` in the .NET SDK 
- The fields `DefinedTags`, `FreeformTags`, `CustomerReferenceName` and `MacsecProperties` were removed from the model `CreateCrossConnectDetails` in the Network Monitoring service in the .NET SDK 
- The field `DomainNameType` was removed from the model `CreateDhcpDetails` in the Network Monitoring service in the .NET SDK 
- The field `RouteTableId` was removed from the models `CreateDrgAttachmentDetails`, `UpdateLocalPeeringGatewayDetails`, `ServiceGateway`, `DrgAttachment`, `InternetGateway`, `CreateLocalPeeringGatewayDetails` and `CreateInternetGatewayDetails` in the Network Monitoring service in the .NET SDK 
- The fields `StaticRoutes`, `CpeLocalIdentifier`, `TunnelConfiguration` and `TunnelCount` were removed from the model `CreateIpSecConnectionDetails` in the Network Monitoring service in the .NET SDK 
- The fields `Ipv6CidrBlocks` and `ProhibitInternetIngress` were removed from the model `CreateSubnetDetails` in the Network Monitoring service in the .NET SDK 
- The field `Ipv6PrivateCidrBlocks`, `CidrBlocks`, `IsOracleGuaAllocationEnabled`, and `Byoipv6CidrDetails` were removed from the model `CreateVcnDetails` in the Network Monitoring service in the .NET SDK 
- The fields `DefinedTags`, `FreeformTags`, `CustomerReferenceName`, `MacsecProperties`, `OciPhysicalDeviceName` and `OciLogicalDeviceName` were removed from the models `CrossConnectGroup` and `CrossConnect` in the Network Monitoring service in the .NET SDK 
- The fields `CustomerBgpPeeringIpv6` and `OracleBgpPeeringIpv6` were removed from the model `CrossConnectMapping` in the Network Monitoring service in the .NET SDK 
- The field `DomainNameType` was removed from the model `DhcpOptions` in the Network Monitoring service in the .NET SDK 
- The fields `CustomerAsnManagement`, `ProviderServiceKeyManagement`, `BandwithShapeManagement`, and `RequiredTotalCrossConnects` were removed from the model `FastConnectProviderService` in the Network Monitoring service in the .NET SDK 
- The field `Description` was removed from the model `IngressSecurityRule` in the Network Monitoring service in the .NET SDK 
- The fields `PeerId`, `PeerAdvertisedCidr`, `PeerAdvertisedCidrDetails`, and `RouteTableId` were removed from the model `LocalPeeringGateway` in the Network Monitoring service in the .NET SDK 
- The field `VlanId` was removed from the model `PrivateIp` in the Network Monitoring service in the .NET SDK 
- The fields `AssignedEntityId`, `AssignedEntityType`, and `PublicIpPoolId` were removed from the model `PublicIp` in the Network Monitoring service in the .NET SDK 
- The fields `FreeformTags` and `DefinedTags` were removed from the models `RemotePeeringConnection` and `UpdateRemotePeeringConnectionDetails` in the Network Monitoring service in the .NET SDK 
- The fields `Description` and `RouteType` were removed from the model `RouteRule` in the Network Monitoring service in the .NET SDK 
- The fields `ProhibitInternetIngress` and `AvailabilityDomain` were removed from the model `Subnet` in the Network Monitoring service in the .NET SDK 
- The fields `DefinedTags`, `FreeformTags`, `CustomerReferenceName` and `MacsecProperties` were removed from the models `UpdateCrossConnectDetails` and `UpdateCrossConnectGroupDetails` in the Network Monitoring service in the .NET SDK 
- The fields `CpeLocalIdentifier`, `CpeLocalIdentifierType`, and `StaticRoutes` were removed from the model `UpdateIpSecConnectionDetails` in the Network Monitoring service in the .NET SDK 
- The fields `RoutingPolicy`, `BgpAdminState`, `IsBfdEnabled`, and `CustomerBgpAsn` were removed from the model `UpdateVirtualCircuitDetails` and `VirtualCircuit` in the Network Monitoring service in the .NET SDK 
- The field `VcnRouteType` was removed from the models `VcnDrgAttachmentNetworkCreateDetails`, `VcnDrgAttachmentNetworkDetails` and `VcnDrgAttachmentNetworkUpdateDetails` in the Network Monitoring service in the .NET SDK 
- The fields `CidrBlocks`, `Byoipv6CidrBlocks`, and `Ipv6PrivateCidrBlocks` were removed from the model `Vcn` in the Network Monitoring service in the .NET SDK 
- The fields `VlanId` and `SubnetId` were removed from the model `Vnic` in the Network Monitoring service in the .NET SDK
...
### Breaking Changes 
- The mandatory field `CompartmentId` was added to the model `ListOpensearchClusterShapesRequest` in the OpenSearch service in the .NET SDK 
- The field `Size` was made mandatory in the model `CreateVirtualNodePoolDetails` in the Kubernetes Engine service in the .NET SDK
...
### Breaking Changes 
- The return type of property `CvssScoreGreaterThan` and `CvssScoreLessThan` was changed to `float` in `Invoke-OCIJmsSummarizeLibraryUsage` in the Java Management Service
...
### Breaking Changes 
- Enum value `EnterpriseEditionExtreme` was renamed to `EnterpriseEditionExtremePerformance` in the Database service in the .NET SDK
...
### Breaking Changes 
- The models `GenericArtifactLocationDetails`, `GenericArtifacts`, `ObjectStorageGenericArtifactLocationDetails`, `StageOutput`, `TestReportOutput` were removed from the Build service in the .NET SDK 
- The enum model `LifecycleStateEnum` was renamed to `OnPremConnectorLifecycleStateEnum` in the Data Safe service in the .NET SDK 
- The enum model `ListOnPremConnectorsOnPremConnectorLifecycleStateEnum` was renamed to `ListOnPremConnectorsLifecycleStateEnum` in the Data Safe service in the .NET SDK 
- The enum values `schemaName` and `tableName` removed from model `ListMaskingPolicyReferentialRelationsSortByEnum` in the Data Safe service in the .NET SDK 
- The field `OpcRetryToken` was removed from the model `CancelWorkRequestRequest` in the Data Safe service in the .NET SDK 
- The field `ComputeHostgroupId` was removed from the models `launchIntanceDetails`, `Instance`, and `LicensingConfig` in the Compute service in the .NET SDK 
- The model `SubscriptionInfo` was removed from the the Cloud Incident Management service in the .NET SDK
...
### Breaking Changes 
- The return type of property `LanguageCode` was changed to `string` in `VoiceSummary` model in the AI Speech service in the .NET SDK
...
### Breaking Changes 
- The type of field `state` has been changed from String to enum `StateEnum` in the models `AutoActivateToggleStatus` and `AutoActivateStatus` in the Application Performance Monitoring service in the .NET SDK
...
### Breaking Changes 
- The values for `ATTACHING` and `DETACHING` were removed from the enum models `LifecycleStateEnum`, `ListNetworkFirewallPoliciesLifecycleStateEnum`, and `ListNetworkFirewallsLifecycleStateEnum` in the Network Firewall service in the .NET SDK
...
### Breaking Changes 
- The operations `GetWorkRequest`, `ListWorkRequestErrors`, `ListWorkRequestLogs`, `ListWorkRequests` were removed from the `FleetAppsManagementClient` in the Fleet Application Management service in the .NET SDK 
- The operations `ListWorkRequestErrorsResponseEnumerator`, `ListWorkRequestErrorsRecordEnumerator`, `ListWorkRequestLogsResponseEnumerator`, `ListWorkRequestsResponseEnumerator`, `ListWorkRequestsRecordEnumerator` were removed from `FleetAppsManagementPaginators` in the Fleet Application Management service in the .NET SDK 
- The operation `ForWorkRequest` was removed from `FleetAppsManagementWaiters` in the Fleet Application Management service in the .NET SDK 
- The properties `ResourceId`, `Type`, `ApplicationType`, `Product`, `LifecycleOperation`, `RunbookId`, `TargetId`, `Subjects` were removed from the model `ActionGroup` in the Fleet Application Management service in the .NET SDK 
- The properties `ResourceId`, `Type`, `Name`, `ApplicationType`, `RunbookId` were removed from the model `ActionGroupDetails` in the Fleet Application Management service in the .NET SDK 
- The property `TenancyId` was removed from the models `ListFleetResourcesRequest` and `AssociatedSchedulerDefinition` in the Fleet Application Management service in the .NET SDK 
- The property `PatchType` was removed from the models `CompliancePolicyRule`, `CompliancePolicyRuleSummary` and `UpdateCompliancePolicyRuleDetails` in the Fleet Application Management service in the .NET SDK 
- The property `Condition` was removed from the model `ComponentProperties` in the Fleet Application Management service in the .NET SDK 
- The type of property `RunOn` was changed from `string` to `RunOnDetails` in the Fleet Application Management service in the .NET SDK 
- The properties `PatchType` and `CompartmentId` were removed from the model `CreateCompliancePolicyRuleDetails` in the Fleet Application Management service in the .NET SDK 
- The property `CompartmentId` was removed from the models `CreateFleetCredentialDetails`, `CreateFleetPropertyDetails`, `ListFleetPropertiesRequest`, `ListFleetCredentialsRequest`, `ListScheduledFleetsRequest` and `UpdatePatchDetails` in the Fleet Application Management service in the .NET SDK 
- The properties `FleetType`, `ApplicationType`, `GroupType`, `ResourceSelectionType`, and `RuleSelectionCriteria` were removed from the model `CreateFleetDetails` in the Fleet Application Management service in the .NET SDK 
- The type of property `NotificationPreferences` was changed from `NotificationPreferences` to a List of `NotificationPreference` in the models `Fleet`, `CreateFleetDetails` and `UpdateFleetDetails` in the Fleet Application Management service in the .NET SDK 
- The properties `RunbookRelevance` and `Associations` were removed from the models `CreateRunbookDetails` and `UpdateRunbookDetails` in the Fleet Application Management service in the .NET SDK 
- The properties `ActivityInitiationCutOff` were removed from the model `CreateSchedulerDefinitionDetails` in the Fleet Application Management service in the .NET SDK 
- The enums `FleetTypeEnum`, `ResourceSelectionTypeEnum`, and `GroupTypeEnum` were removed from the model `Fleet` in the Fleet Application Management service in the .NET SDK 
- The properties `FleetType`, `ApplicationType`, `GroupType`, `ResourceSelectionType`, and `RuleSelectionCriteria` were removed from the model `Fleet` in the Fleet Application Management service in the .NET SDK 
- The property `ApplicationType` was removed from the models `FleetResource` and `FleetResourceSummary` in the Fleet Application Management service in the .NET SDK 
- The property `FleetType` was removed from the model `FleetSummary` in the Fleet Application Management service in the .NET SDK 
- The properties `MaintenanceWindowType` and `TaskInitiationCutoff` were removed from the models `MaintenanceWindow`, `MaintenanceWindowSummary`, `CreateMaintenanceWindowDetails` and `UpdateMaintenanceWindowDetails` in the Fleet Application Management service in the .NET SDK 
- The property `Id` was removed from the model `OperationRunbook` in the Fleet Application Management service in the .NET SDK 
- The type of property `Content` was changed from `ContentDetails` to `PatchFileContentDetails` in the models `GenericArtifact` and `PlatformSpecificArtifact` in the Fleet Application Management service in the .NET SDK 
- The property `OnUpcomingSchedule` was removed from the model `Preferences` in the Fleet Application Management service in the .NET SDK 
- The enum `RunbookRelevanceEnum`, and properties `RunbookRelevance` and `Associations` were removed from the model `Runbook` in the Fleet Application Management service in the .NET SDK 
- The property `RunbookRelevance` was removed from the models `RunbookSummary` and `ListRunbooksRequest` in the Fleet Application Management service in the .NET SDK 
- The properties `Type`, `MaintenanceWindowId`, `Recurrences`, and `Duration` were removed from the model `Schedule` in the Fleet Application Management service in the .NET SDK 
- The properties `TenancyId`, `ActionGroupTypes`, and `ApplicationTypes` were removed from the model `ScheduledFleetSummary` in the Fleet Application Management service in the .NET SDK 
- The properties `ActivityInitiationCutOff`, `ActionGroupTypes` and `ApplicationTypes` were removed from the model `SchedulerDefinition` in the Fleet Application Management service in the .NET SDK 
- The properties `ActionGroupTypes` and `ApplicationTypes` were removed from the model `SchedulerDefinitionSummary` in the Fleet Application Management service in the .NET SDK 
- The enum members `Active` and `Deleted` were removed from the enum `LifecycleStateEnum` in the model `SchedulerJob` in the Fleet Application Management service in the .NET SDK 
- The properties `ActionGroupTypes`, `ApplicationTypes` and `AssociatedScheduleDefinition` were removed from the models `SchedulerJob` and `SchedulerJobSummary` in the Fleet Application Management service in the .NET SDK 
- The enum `AssociationTypeEnum` and the property `AssociationType` were removed from the model `Task` in the Fleet Application Management service in the .NET SDK 
- The property `Value` was removed in the model `TaskArgument` in the Fleet Application Management service in the .NET SDK 
- The property `RuleSelectionCriteria` was removed from the model `UpdateFleetDetails` in the Fleet Application Management service in the .NET SDK 
- The property `ActivityInitiationCutOff` was removed from the model `UpdateSchedulerDefinitionDetails` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `CompliancePolicyRuleSortBy` in the model `ListCompliancePolicyRulesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `CompliancePolicySortBy` in the model `ListCompliancePoliciesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `ComplianceRecordSortBy` in the model `ListComplianceRecordsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `FleetCredentialSortBy` in the model `ListFleetCredentialsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `ProductSortBy` in the model `ListFleetProductsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `FleetPropertySortBy` in the model `ListFleetPropertiesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `FleetResourceSortBy` in the model `ListFleetResourcesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `FleetSortBy` in the model `ListFleetsRequest` in the Fleet Application Management service in the .NET SDK 
- The type of property `FleetType` from `Fleet.FleetTypeEnum` to `FleetDetails.FleetTypeEnum` in the model `ListFleetsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `TargetSortBy` in the model `ListFleetTargetsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `InventoryResourceSortBy` in the model `ListInventoryResourcesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `MaintenanceWindowSortBy` in the model `ListMaintenanceWindowsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `SortBy` in the model `ListOnboardingsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `PatchSortBy` in the model `ListPatchesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `PlatformConfigurationSortBy` in the model `ListPlatformConfigurationsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `PropertySortBy` in the model `ListPropertiesRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `RunbookSortBy` in the model `ListRunbooksRequest` in the Fleet Application Management service in the .NET SDK 
- The properties `SubState` and `DefintionId` were removed from the model `ListSchedulerJobsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `SortBy` in the model `ListTargetsRequest` in the Fleet Application Management service in the .NET SDK 
- The enum `SortByEnum` was removed and the type of property `SortBy` was changed from `SortByEnum` to `TaskRecordSortBy` in the model `ListTaskRecordsRequest` in the Fleet Application Management service in the .NET SDK
...
### Breaking Changes 
- The type of properties `Tasks`, `Target`, and `Source` were changed to `TaskDetailsResponse`, `TargetDetailsResponse`, and `SourceDetailsResponse` respectively in the model `ServiceConnector` in the Resource Scheduler service in the .NET SDK
...
### Breaking Changes 
- The type of property `SizeInBytes` was changed from `int` to `long` in the model `KnowledgeBaseStatistics` in the Generative AI Agent service in the .NET SDK
...
### Breaking Changes 
- The properties `IpAnycastId` and `MonitorIp` were removed from the models `CreateByoipRangeDetails` and `UpdateByoipRangeDetails` in the Compute service in the .NET SDK
...
### Breaking Changes 
- The properties `MaxMemoryGB`, `MaxOcpuCount`, `MinMemoryGB`, `MinOcpuCount` were removed from the models `CreateOpensearchClusterPipelineDetails`, `OpensearchClusterPipeline`, `OpensearchClusterPipelineSummary`, and `UpdateOpensearchClusterPipelineDetails` in the OpenSearch service in the .NET SDK 
- The property `SystemTags` was removed from the model `CreateOpensearchClusterPipelineDetails` in the OpenSearch service in the .NET SDK 
- The models `OpensearchPipelineValidationResponse`, `ValidateOpensearchPipelineDetails`, and `PipelineValidationErrorDetails` were removed in the OpenSearch service in the .NET SDK
...
### Breaking Changes 
- The property `OpcWorkRequestId` was removed from the model `CreateSubscriptionMappingResponse` in the Organizations service in the .NET SDK
...
### Breaking Changes 
- The models `CreateAuditPolicyDetails` and `CreateAuditProfileDetails` were removed in the Data Safe service in the .NET SDK
...
### Breaking Changes 
- The enum ClassicSubscriptionEnvironmentName was removed from the Organizations service in the .NET SDK
...
### Breaking Changes 
- The model `ServiceCategories` was removed in the Customer Incident Management Service in the .NET SDK 
- The property `SubComponents` was removed from the `SubCategories` model in the Customer Incident Management Service in the .NET SDK 
- The property `LimitStatus` and the enum `LimitStatusEnum` were removed from the model `CreateLimitItemDetails` in the Customer Incident Management Service in the .NET SDK 
- The type of property `Services` was changed to list of `Services` from a list of `ServiceCategories` in the Customer Incident Management Service in the .NET SDK
...
### Breaking Changes 
- The cmdlet `UpdateOCICloudbridgePlugin` was removed from the Cloud Bridge service 
- The property `DiscoveryScheduleId` was removed from the model `UpdateVmWareAssetSourceDetails` in the Cloud Bridge service in the .NET SDK 
- The enum `HOST_CONTAINERS` was removed from the enum `MetricNameEnum` in the model `HostPerformanceMetricGroup` in the Operations Insight service in the .NET SDK 
- The parent class of model `HostContainers` was changed from `HostPerformanceMetricGroup` to `HostConfigurationMetricGroup` in the Operations Insight service in the .NET SDK
...
- The properties AgentId, SourceContainerDatabaseConnectionId, ExcludeObjects, IncludeObjects and VaultDetails were removed from the model CloneMigrationDetails in the Database Migration service in the .NET SDK
