...
{
    [Cmdlet("New", "OCIComputeGpuMemoryCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.ComputeGpuMemoryCluster), typeof(Oci.CoreService.Responses.CreateComputeGpuMemoryClusterResponse) })]
    public class NewOCIComputeGpuMemoryCluster : OCIComputeCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The configuration details of a GPU memory cluster", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The configuration details of a GPU memory cluster", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateComputeGpuMemoryClusterRequest
                {
        ...
        private void HandleOutput(CreateComputeGpuMemoryClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateComputeGpuMemoryCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateComputeGpuMemoryCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ComputeGpuMemoryCluster);
        }

        private CreateComputeGpuMemoryClusterResponse response;
