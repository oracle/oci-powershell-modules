...
{
    [Cmdlet("New", "OCIDatabaseExecutionAction", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExecutionAction), typeof(Oci.DatabaseService.Responses.CreateExecutionActionResponse) })]
    public class NewOCIDatabaseExecutionAction : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create execution action.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create execution action.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateExecutionActionRequest
                {
        ...
        private void HandleOutput(CreateExecutionActionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateExecutionAction(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateExecutionAction(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExecutionAction);
        }

        private CreateExecutionActionResponse response;
