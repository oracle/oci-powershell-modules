...
{
    [Cmdlet("Invoke", "OCIOpsiIngestHostMetrics")]
    [OutputType(new System.Type[] { typeof(Oci.OpsiService.Models.IngestHostMetricsResponseDetails), typeof(Oci.OpsiService.Responses.IngestHostMetricsResponse) })]
    public class InvokeOCIOpsiIngestHostMetrics : OCIOperationsInsightsCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Payload for one or more host performance metrics for a particular host.")]
...
*Note:* Retry tokens can be invalidated before the 24 hour time limit due to conflicting operations, such as a resource being deleted or purged from the system.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new IngestHostMetricsRequest
                {
                    Id = Id,
                ...
                response = client.IngestHostMetrics(request).GetAwaiter().GetResult();
                WriteOutput(response, response.IngestHostMetricsResponseDetails);
        ...
        private IngestHostMetricsResponse response;
    }
}
