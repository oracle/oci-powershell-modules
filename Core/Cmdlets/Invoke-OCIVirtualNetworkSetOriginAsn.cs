...
{
    [Cmdlet("Invoke", "OCIVirtualNetworkSetOriginAsn", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.SetOriginAsnResponse) })]
    public class InvokeOCIVirtualNetworkSetOriginAsn : OCIVirtualNetworkCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the `ByoipRange` resource containing the BYOIP CIDR block.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the `ByoipRange` resource containing the BYOIP CIDR block.", ParameterSetName = Default)]
        public string ByoipRangeId { get; set; }

        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"ASN details", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"ASN details", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new SetOriginAsnRequest
                {
        ...
        private void HandleOutput(SetOriginAsnRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForSetOriginAsn(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.SetOriginAsn(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private SetOriginAsnResponse response;
