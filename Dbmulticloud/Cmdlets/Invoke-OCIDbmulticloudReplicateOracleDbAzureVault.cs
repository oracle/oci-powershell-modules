...
{
    [Cmdlet("Invoke", "OCIDbmulticloudReplicateOracleDbAzureVault")]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DbmulticloudService.Responses.ReplicateOracleDbAzureVaultResponse) })]
    public class InvokeOCIDbmulticloudReplicateOracleDbAzureVault : OCIOracleDbAzureVaultCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for to replicate Oracle DB Azure Vault resource to target region.")]
...
For example, if a resource has been deleted and permanently purged from the system, a retry of the original creation request may be rejected.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ReplicateOracleDbAzureVaultRequest
                {
                ...
                response = client.ReplicateOracleDbAzureVault(request).GetAwaiter().GetResult();
        ...
        private ReplicateOracleDbAzureVaultResponse response;
    }
}
