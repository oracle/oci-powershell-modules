...
{
    [Cmdlet("Enable", "OCIDatabasePluggableDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabase), typeof(Oci.DatabaseService.Responses.EnablePluggableDatabaseManagementResponse) })]
    public class EnableOCIDatabasePluggableDatabaseManagement : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management service for an Oracle Pluggable Database located in Oracle Cloud Infrastructure.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to enable the Database Management service for an Oracle Pluggable Database located in Oracle Cloud Infrastructure.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnablePluggableDatabaseManagementRequest
                {
        ...
        private void HandleOutput(EnablePluggableDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnablePluggableDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnablePluggableDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabase);
        }

        private EnablePluggableDatabaseManagementResponse response;
