...
{
    [Cmdlet("Get", "OCIDatabasemanagementManagedDatabasesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.ManagedDatabaseCollection), typeof(Oci.DatabasemanagementService.Responses.ListManagedDatabasesResponse) })]
    public class GetOCIDatabasemanagementManagedDatabasesList : OCIDbManagementCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The [OCID](https://docs.oracle.com/Content/General/Concepts/identifiers.htm) of the Exadata infrastructure.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListManagedDatabasesRequest
                {
                ...
                IEnumerable<ListManagedDatabasesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ManagedDatabaseCollection, true);
                }
        ...
        {
            IEnumerable<ListManagedDatabasesResponse> DefaultRequest(ListManagedDatabasesRequest request) => Enumerable.Repeat(client.ListManagedDatabases(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListManagedDatabasesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListManagedDatabasesResponse response;
        private delegate IEnumerable<ListManagedDatabasesResponse> RequestDelegate(ListManagedDatabasesRequest request);
