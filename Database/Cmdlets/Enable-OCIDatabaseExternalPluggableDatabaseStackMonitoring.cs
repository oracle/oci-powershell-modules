...
{
    [Cmdlet("Enable", "OCIDatabaseExternalPluggableDatabaseStackMonitoring", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.EnableExternalPluggableDatabaseStackMonitoringResponse) })]
    public class EnableOCIDatabaseExternalPluggableDatabaseStackMonitoring : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to enable Stack Monitoring on the external pluggable database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details to enable Stack Monitoring on the external pluggable database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new EnableExternalPluggableDatabaseStackMonitoringRequest
                {
        ...
        private void HandleOutput(EnableExternalPluggableDatabaseStackMonitoringRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForEnableExternalPluggableDatabaseStackMonitoring(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.EnableExternalPluggableDatabaseStackMonitoring(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private EnableExternalPluggableDatabaseStackMonitoringResponse response;
