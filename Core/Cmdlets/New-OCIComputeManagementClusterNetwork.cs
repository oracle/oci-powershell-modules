...
{
    [Cmdlet("New", "OCIComputeManagementClusterNetwork", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.CoreService.Models.ClusterNetwork), typeof(Oci.CoreService.Responses.CreateClusterNetworkResponse) })]
    public class NewOCIComputeManagementClusterNetwork : OCIComputeManagementCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Cluster network creation details", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Cluster network creation details", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateClusterNetworkRequest
                {
        ...
        private void HandleOutput(CreateClusterNetworkRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateClusterNetwork(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateClusterNetwork(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ClusterNetwork);
        }

        private CreateClusterNetworkResponse response;
