...
{
    [Cmdlet("Get", "OCIDevopsDeployArtifactsList")]
    [OutputType(new System.Type[] { typeof(Oci.DevopsService.Models.DeployArtifactCollection), typeof(Oci.DevopsService.Responses.ListDeployArtifactsResponse) })]
    public class GetOCIDevopsDeployArtifactsList : OCIDevopsCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to return only DeployArtifacts that matches the given lifecycleState.")]
        public System.Nullable<Oci.DevopsService.Models.DeployArtifact.LifecycleStateEnum> LifecycleState { get; set; }
        ...
        public System.Nullable<Oci.DevopsService.Requests.ListDeployArtifactsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListDeployArtifactsRequest
                {
                    Id = Id,
                    ProjectId = ProjectId,
                ...
                IEnumerable<ListDeployArtifactsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.DeployArtifactCollection, true);
                }
        ...
        {
            IEnumerable<ListDeployArtifactsResponse> DefaultRequest(ListDeployArtifactsRequest request) => Enumerable.Repeat(client.ListDeployArtifacts(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListDeployArtifactsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListDeployArtifactsResponse response;
        private delegate IEnumerable<ListDeployArtifactsResponse> RequestDelegate(ListDeployArtifactsRequest request);
