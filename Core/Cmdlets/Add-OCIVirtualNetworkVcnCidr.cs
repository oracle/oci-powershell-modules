...
{
    [Cmdlet("Add", "OCIVirtualNetworkVcnCidr", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.AddVcnCidrResponse) })]
    public class AddOCIVirtualNetworkVcnCidr : OCIVirtualNetworkCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for deleting a VCN CIDR.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details object for deleting a VCN CIDR.", ParameterSetName = Default)]
        ...
        private void HandleOutput(AddVcnCidrRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForAddVcnCidr(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.AddVcnCidr(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private AddVcnCidrResponse response;
