...
{
    [Cmdlet("New", "OCIDatabaseSoftwareImage", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DatabaseSoftwareImage), typeof(Oci.DatabaseService.Responses.CreateDatabaseSoftwareImageResponse) })]
    public class NewOCIDatabaseSoftwareImage : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create database software image.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create database software image.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateDatabaseSoftwareImageRequest
                {
        ...
        private void HandleOutput(CreateDatabaseSoftwareImageRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateDatabaseSoftwareImage(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateDatabaseSoftwareImage(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DatabaseSoftwareImage);
        }

        private CreateDatabaseSoftwareImageResponse response;
