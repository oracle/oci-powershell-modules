...
{
    [Cmdlet("Get", "OCIManagementdashboard", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.ManagementdashboardService.Models.ManagementDashboard), typeof(Oci.ManagementdashboardService.Responses.GetManagementDashboardResponse) })]
    public class GetOCIManagementdashboard : OCIDashxApisCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"A unique dashboard identifier.", ParameterSetName = LifecycleStateParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"A unique dashboard identifier.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new GetManagementDashboardRequest
                {
        ...
        private void HandleOutput(GetManagementDashboardRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForManagementDashboard(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetManagementDashboard(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ManagementDashboard);
        }

        private GetManagementDashboardResponse response;
