...
{
    [Cmdlet("Edit", "OCIDatabaseManagement", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.Database), typeof(Oci.DatabaseService.Responses.ModifyDatabaseManagementResponse) })]
    public class EditOCIDatabaseManagement : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The data to update one or more attributes of the Database Management Service for the database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The data to update one or more attributes of the Database Management Service for the database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ModifyDatabaseManagementRequest
                {
        ...
        private void HandleOutput(ModifyDatabaseManagementRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForModifyDatabaseManagement(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ModifyDatabaseManagement(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.Database);
        }

        private ModifyDatabaseManagementResponse response;
