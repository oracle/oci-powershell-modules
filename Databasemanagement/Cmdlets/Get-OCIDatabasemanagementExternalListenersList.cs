...
{
    [Cmdlet("Get", "OCIDatabasemanagementExternalListenersList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.ExternalListenerCollection), typeof(Oci.DatabasemanagementService.Responses.ListExternalListenersResponse) })]
    public class GetOCIDatabasemanagementExternalListenersList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListExternalListenersRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListExternalListenersRequest
                {
                ...
                IEnumerable<ListExternalListenersResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.ExternalListenerCollection, true);
                }
        ...
        {
            IEnumerable<ListExternalListenersResponse> DefaultRequest(ListExternalListenersRequest request) => Enumerable.Repeat(client.ListExternalListeners(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListExternalListenersResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListExternalListenersResponse response;
        private delegate IEnumerable<ListExternalListenersResponse> RequestDelegate(ListExternalListenersRequest request);
