...
{
    [Cmdlet("Invoke", "OCIVirtualNetworkBulkDetachIpv6s", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.CoreService.Responses.BulkDetachIpv6sResponse) })]
    public class InvokeOCIVirtualNetworkBulkDetachIpv6s : OCIVirtualNetworkCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details needed to detach IPv6s in bulk.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details needed to detach IPv6s in bulk.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new BulkDetachIpv6sRequest
                {
        ...
        private void HandleOutput(BulkDetachIpv6sRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForBulkDetachIpv6s(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.BulkDetachIpv6s(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private BulkDetachIpv6sResponse response;
