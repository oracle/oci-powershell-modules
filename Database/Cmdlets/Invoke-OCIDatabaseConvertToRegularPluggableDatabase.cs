...
{
    [Cmdlet("Invoke", "OCIDatabaseConvertToRegularPluggableDatabase", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.PluggableDatabase), typeof(Oci.DatabaseService.Responses.ConvertToRegularPluggableDatabaseResponse) })]
    public class InvokeOCIDatabaseConvertToRegularPluggableDatabase : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to convert a Refreshable clone pluggable database (PDB) to a Regular pluggable database.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to convert a Refreshable clone pluggable database (PDB) to a Regular pluggable database.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ConvertToRegularPluggableDatabaseRequest
                {
        ...
        private void HandleOutput(ConvertToRegularPluggableDatabaseRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForConvertToRegularPluggableDatabase(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ConvertToRegularPluggableDatabase(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.PluggableDatabase);
        }

        private ConvertToRegularPluggableDatabaseResponse response;
