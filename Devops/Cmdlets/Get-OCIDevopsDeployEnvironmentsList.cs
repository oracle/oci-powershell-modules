...
{
    [Cmdlet("Get", "OCIDevopsDeployEnvironmentsList")]
    [OutputType(new System.Type[] { typeof(Oci.DevopsService.Models.DeployEnvironmentCollection), typeof(Oci.DevopsService.Responses.ListDeployEnvironmentsResponse) })]
    public class GetOCIDevopsDeployEnvironmentsList : OCIDevopsCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"A filter to return only DeployEnvironments that matches the given lifecycleState.")]
        public System.Nullable<Oci.DevopsService.Models.DeployEnvironment.LifecycleStateEnum> LifecycleState { get; set; }
        ...
        public System.Nullable<Oci.DevopsService.Requests.ListDeployEnvironmentsRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListDeployEnvironmentsRequest
                {
                    ProjectId = ProjectId,
                ...
                IEnumerable<ListDeployEnvironmentsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.DeployEnvironmentCollection, true);
                }
        ...
        {
            IEnumerable<ListDeployEnvironmentsResponse> DefaultRequest(ListDeployEnvironmentsRequest request) => Enumerable.Repeat(client.ListDeployEnvironments(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListDeployEnvironmentsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListDeployEnvironmentsResponse response;
        private delegate IEnumerable<ListDeployEnvironmentsResponse> RequestDelegate(ListDeployEnvironmentsRequest request);
