...
{
    [Cmdlet("Move", "OCIDatabaseExadbVmClusterSubscription", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeExadbVmClusterSubscriptionResponse) })]
    public class MoveOCIDatabaseExadbVmClusterSubscription : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a Exadata VM cluster on Exascale Infrastructure with a different subscription.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Associate a Exadata VM cluster on Exascale Infrastructure with a different subscription.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeExadbVmClusterSubscriptionRequest
                {
        ...
        private void HandleOutput(ChangeExadbVmClusterSubscriptionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeExadbVmClusterSubscription(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeExadbVmClusterSubscription(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeExadbVmClusterSubscriptionResponse response;
