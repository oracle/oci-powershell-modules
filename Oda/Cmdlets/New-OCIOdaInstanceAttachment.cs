...
{
    [Cmdlet("New", "OCIOdaInstanceAttachment")]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.OdaService.Responses.CreateOdaInstanceAttachmentResponse) })]
    public class NewOCIOdaInstanceAttachment : OCIOdaCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Details for the new Digital Assistant instance attachment.")]
...
Retry tokens expire after 24 hours, but they can become invalid before then if there are conflicting operations. For example, if an instance was deleted and purged from the system, then the service might reject a retry of the original creation request.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateOdaInstanceAttachmentRequest
                {
                ...
                response = client.CreateOdaInstanceAttachment(request).GetAwaiter().GetResult();
        ...
        private CreateOdaInstanceAttachmentResponse response;
    }
}
