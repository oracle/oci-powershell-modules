...
{
    [Cmdlet("New", "OCIDatabaseBaseccVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.BaseccVmCluster), typeof(Oci.DatabaseService.Responses.CreateBaseccVmClusterResponse) })]
    public class NewOCIDatabaseBaseccVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a BaseDB-C@C VM cluster. Applies to Base Database Service on Cloud@Customer instances only.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a BaseDB-C@C VM cluster. Applies to Base Database Service on Cloud@Customer instances only.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateBaseccVmClusterRequest
                {
        ...
        private void HandleOutput(CreateBaseccVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateBaseccVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateBaseccVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.BaseccVmCluster);
        }

        private CreateBaseccVmClusterResponse response;
