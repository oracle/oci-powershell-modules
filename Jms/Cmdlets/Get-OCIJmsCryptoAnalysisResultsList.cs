...
{
    [Cmdlet("Get", "OCIJmsCryptoAnalysisResultsList")]
    [OutputType(new System.Type[] { typeof(Oci.JmsService.Models.CryptoAnalysisResultCollection), typeof(Oci.JmsService.Responses.ListCryptoAnalysisResultsResponse) })]
    public class GetOCIJmsCryptoAnalysisResultsList : OCIJavaManagementServiceCmdlet
    {
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"The aggregation mode of the crypto event analysis result.")]
        public System.Nullable<Oci.JmsService.Models.CryptoAnalysisResultMode> AggregationMode { get; set; }
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Non Compliant Finding Count of CryptoAnalysis Report.")]
        ...
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"FindingCount of CryptoAnalysis Report.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCryptoAnalysisResultsRequest
                {
                    FleetId = FleetId,
                ...
                IEnumerable<ListCryptoAnalysisResultsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CryptoAnalysisResultCollection, true);
                }
        ...
        {
            IEnumerable<ListCryptoAnalysisResultsResponse> DefaultRequest(ListCryptoAnalysisResultsRequest request) => Enumerable.Repeat(client.ListCryptoAnalysisResults(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCryptoAnalysisResultsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCryptoAnalysisResultsResponse response;
        private delegate IEnumerable<ListCryptoAnalysisResultsResponse> RequestDelegate(ListCryptoAnalysisResultsRequest request);
