...
{
    [Cmdlet("Invoke", "OCIDatabaseRotateAutonomousVmClusterSslCerts", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.RotateAutonomousVmClusterSslCertsResponse) })]
    public class InvokeOCIDatabaseRotateAutonomousVmClusterSslCerts : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to rotate the SSL certificates on Autonomous Exadata VM cluster.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to rotate the SSL certificates on Autonomous Exadata VM cluster.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new RotateAutonomousVmClusterSslCertsRequest
                {
        ...
        private void HandleOutput(RotateAutonomousVmClusterSslCertsRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForRotateAutonomousVmClusterSslCerts(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.RotateAutonomousVmClusterSslCerts(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private RotateAutonomousVmClusterSslCertsResponse response;
