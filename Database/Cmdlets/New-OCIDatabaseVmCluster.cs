...
{
    [Cmdlet("New", "OCIDatabaseVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.VmCluster), typeof(Oci.DatabaseService.Responses.CreateVmClusterResponse) })]
    public class NewOCIDatabaseVmCluster : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a VM cluster. Applies to Exadata Cloud@Customer instances only. See CreateCloudVmClusterDetails for details on creating a cloud VM cluster in an Exadata Cloud Service instance.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to create a VM cluster. Applies to Exadata Cloud@Customer instances only. See CreateCloudVmClusterDetails for details on creating a cloud VM cluster in an Exadata Cloud Service instance.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new CreateVmClusterRequest
                {
        ...
        private void HandleOutput(CreateVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForCreateVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.CreateVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.VmCluster);
        }

        private CreateVmClusterResponse response;
