...
{
    [Cmdlet("Invoke", "OCIOpsiIngestSqlStats")]
    [OutputType(new System.Type[] { typeof(Oci.OpsiService.Models.IngestSqlStatsResponseDetails), typeof(Oci.OpsiService.Responses.IngestSqlStatsResponse) })]
    public class InvokeOCIOpsiIngestSqlStats : OCIOperationsInsightsCmdlet
    {
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Collection of SQL stats objects for a particular database.")]
...
*Note:* Retry tokens can be invalidated before the 24 hour time limit due to conflicting operations, such as a resource being deleted or purged from the system.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new IngestSqlStatsRequest
                {
                ...
                response = client.IngestSqlStats(request).GetAwaiter().GetResult();
                WriteOutput(response, response.IngestSqlStatsResponseDetails);
        ...
        private IngestSqlStatsResponse response;
    }
}
