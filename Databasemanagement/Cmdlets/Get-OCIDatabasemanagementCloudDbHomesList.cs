...
{
    [Cmdlet("Get", "OCIDatabasemanagementCloudDbHomesList")]
    [OutputType(new System.Type[] { typeof(Oci.DatabasemanagementService.Models.CloudDbHomeCollection), typeof(Oci.DatabasemanagementService.Responses.ListCloudDbHomesResponse) })]
    public class GetOCIDatabasemanagementCloudDbHomesList : OCIDbManagementCmdlet
    {
        ...
        public System.Nullable<Oci.DatabasemanagementService.Requests.ListCloudDbHomesRequest.SortByEnum> SortBy { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ListCloudDbHomesRequest
                {
                ...
                IEnumerable<ListCloudDbHomesResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.CloudDbHomeCollection, true);
                }
        ...
        {
            IEnumerable<ListCloudDbHomesResponse> DefaultRequest(ListCloudDbHomesRequest request) => Enumerable.Repeat(client.ListCloudDbHomes(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListCloudDbHomesResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListCloudDbHomesResponse response;
        private delegate IEnumerable<ListCloudDbHomesResponse> RequestDelegate(ListCloudDbHomesRequest request);
