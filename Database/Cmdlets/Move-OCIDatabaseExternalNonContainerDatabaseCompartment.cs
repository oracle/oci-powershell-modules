...
{
    [Cmdlet("Move", "OCIDatabaseExternalNonContainerDatabaseCompartment", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.PSModules.Common.Cmdlets.WorkRequest), typeof(Oci.DatabaseService.Responses.ChangeExternalNonContainerDatabaseCompartmentResponse) })]
    public class MoveOCIDatabaseExternalNonContainerDatabaseCompartment : OCIDatabaseCmdlet
    {
        
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move the external non-container database to a different compartment.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to move the external non-container database to a different compartment.", ParameterSetName = Default)]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new ChangeExternalNonContainerDatabaseCompartmentRequest
                {
        ...
        private void HandleOutput(ChangeExternalNonContainerDatabaseCompartmentRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForChangeExternalNonContainerDatabaseCompartment(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.ChangeExternalNonContainerDatabaseCompartment(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, CreateWorkRequestObject(response.OpcWorkRequestId));
        }

        private ChangeExternalNonContainerDatabaseCompartmentResponse response;
