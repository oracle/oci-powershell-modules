...
{
    [Cmdlet("Update", "OCIDatabaseExadbVmCluster", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.ExadbVmCluster), typeof(Oci.DatabaseService.Responses.UpdateExadbVmClusterResponse) })]
    public class UpdateOCIDatabaseExadbVmCluster : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the attributes of a Exadata VM cluster on Exascale Infrastructure.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"Request to update the attributes of a Exadata VM cluster on Exascale Infrastructure.", ParameterSetName = Default)]
        ...
        private void HandleOutput(UpdateExadbVmClusterRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForUpdateExadbVmCluster(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.UpdateExadbVmCluster(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.ExadbVmCluster);
        }

        private UpdateExadbVmClusterResponse response;
