...
{
    [Cmdlet("Get", "OCIDisasterrecoveryDrPlan", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DisasterrecoveryService.Models.DrPlan), typeof(Oci.DisasterrecoveryService.Responses.GetDrPlanResponse) })]
...
Example: `ocid1.drplan.oc1..uniqueID`", ParameterSetName = Default)]
        ...
        public Oci.DisasterrecoveryService.Models.DrPlanLifecycleState[] WaitForLifecycleState { get; set; }
        ...
        private void HandleOutput(GetDrPlanRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForDrPlan(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetDrPlan(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DrPlan);
        }

        private GetDrPlanResponse response;
