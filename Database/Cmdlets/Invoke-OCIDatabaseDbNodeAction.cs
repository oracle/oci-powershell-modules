...
{
    [Cmdlet("Invoke", "OCIDatabaseDbNodeAction", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DatabaseService.Models.DbNode), typeof(Oci.DatabaseService.Responses.DbNodeActionResponse) })]
    public class InvokeOCIDatabaseDbNodeAction : OCIDatabaseCmdlet
    {
        ...
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The action to perform on the DB Node.", ParameterSetName = StatusParamSet)]
        [Parameter(Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = @"The action to perform on the DB Node.", ParameterSetName = Default)]
        public System.Nullable<Oci.DatabaseService.Requests.DbNodeActionRequest.ActionEnum> Action { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new DbNodeActionRequest
                {
                    DbNodeId = DbNodeId,
                    Action = Action,
        ...
        private void HandleOutput(DbNodeActionRequest request)
        {
            ...
            { 
                case StatusParamSet:
                    response = client.Waiters.ForDbNodeAction(request, waiterConfig, WaitForStatus).Execute();
                    break;

                case Default:
                    response = client.DbNodeAction(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DbNode);
        }

        private DbNodeActionResponse response;
