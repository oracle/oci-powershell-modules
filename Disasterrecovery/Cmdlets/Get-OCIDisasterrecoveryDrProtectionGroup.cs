...
{
    [Cmdlet("Get", "OCIDisasterrecoveryDrProtectionGroup", DefaultParameterSetName = Default)]
    [OutputType(new System.Type[] { typeof(Oci.DisasterrecoveryService.Models.DrProtectionGroup), typeof(Oci.DisasterrecoveryService.Responses.GetDrProtectionGroupResponse) })]
...
Example: `ocid1.drprotectiongroup.oc1..uniqueID`", ParameterSetName = Default)]
        ...
        public Oci.DisasterrecoveryService.Models.DrProtectionGroupLifecycleState[] WaitForLifecycleState { get; set; }
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                request = new GetDrProtectionGroupRequest
                {
        ...
        private void HandleOutput(GetDrProtectionGroupRequest request)
        {
            ...
            { 
                case LifecycleStateParamSet:
                    response = client.Waiters.ForDrProtectionGroup(request, waiterConfig, WaitForLifecycleState).Execute();
                    break;

                case Default:
                    response = client.GetDrProtectionGroup(request).GetAwaiter().GetResult();
                    break;
            }
            WriteOutput(response, response.DrProtectionGroup);
        }

        private GetDrProtectionGroupResponse response;
