...
{
    [Cmdlet("Get", "OCIFleetappsmanagementOnboardingsList")]
    [OutputType(new System.Type[] { typeof(Oci.FleetappsmanagementService.Models.OnboardingCollection), typeof(Oci.FleetappsmanagementService.Responses.ListOnboardingsResponse) })]
    public class GetOCIFleetappsmanagementOnboardingsList : OCIFleetAppsManagementAdminCmdlet
    {
        ...
        public System.Nullable<Oci.FleetappsmanagementService.Models.Onboarding.LifecycleStateEnum> LifecycleState { get; set; }

        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, HelpMessage = @"Unique identifier or OCID for listing a single onboarding by id. Either compartmentId or id must be provided.")]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                ...
                IEnumerable<ListOnboardingsResponse> responses = GetRequestDelegate().Invoke(request);
                foreach (var item in responses)
                {
                    response = item;
                    WriteOutput(response, response.OnboardingCollection, true);
                }
        ...
        {
            IEnumerable<ListOnboardingsResponse> DefaultRequest(ListOnboardingsRequest request) => Enumerable.Repeat(client.ListOnboardings(request).GetAwaiter().GetResult(), 1);
            if (ParameterSetName.Equals(AllPageSet))
            {
                return req => client.Paginators.ListOnboardingsResponseEnumerator(req);
            }
            return DefaultRequest;
        }

        private ListOnboardingsResponse response;
        private delegate IEnumerable<ListOnboardingsResponse> RequestDelegate(ListOnboardingsRequest request);
